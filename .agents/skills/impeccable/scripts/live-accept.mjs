...
Output (JSON):
  ...
  if (!isDiscard && !variantNum) {
    console.error('Choose an action: --discard or --variant N.');
    process.exit(1);
  }
  ...
  if (!found) {
    console.log(
      JSON.stringify({ handled: false, error: 'No variant markers found for session id: ' + id })
    );
    process.exit(0);
  }
  ...
  if (isGeneratedFile(targetFile, { cwd: process.cwd() })) {
    console.log(
      JSON.stringify({
        ...
        mode: 'fallback',
        file: relFile,
        hint: 'Session is in generated output. Persist the accepted variant in source; do not rely on this script.',
      })
    );
    process.exit(0);
  }
