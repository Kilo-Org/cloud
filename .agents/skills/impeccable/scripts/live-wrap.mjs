...
The agent should insert variant HTML at insertLine.`);
  ...
  if (!id) {
    console.error('Pass --id with the live session id.');
    process.exit(1);
  }
  if (!elementId && !classes && !query) {
    console.error('Pass at least one source locator: --element-id, --classes, or --query.');
    process.exit(1);
  }
      ...
      if (generatedHit) {
        console.error(
          JSON.stringify({
            ...
            hint: 'Element only exists in generated output. Update the source component manually; see "Handle fallback" in live.md.',
          })
        );
      } else {
        console.error(
          JSON.stringify({
            ...
            fallback: 'agent-driven',
            hint: 'Element was not found in source. It may be rendered at runtime; see "Handle fallback" in live.md.',
          })
        );
      }
      process.exit(1);
    ...
    if (isGeneratedFile(targetFile, genOpts)) {
      console.error(
        JSON.stringify({
          ...
          hint: 'The --file target is generated output. Update the source component instead; see "Handle fallback" in live.md.',
        })
      );
      process.exit(1);
    }
  ...
  if (text) {
    ...
    if (candidates.length === 0) {
      console.error(
        JSON.stringify({
          error:
            'Found source file but could not locate element in ' +
            targetFile +
            '. Searched for: ' +
            queries.join(', '),
        })
      ...
      if (filtered.length === 1) {
        ...
        console.error(
          JSON.stringify({
            ...
            hint: 'Multiple source elements match. Pass --element-id, a more specific --text, or write the wrapper manually. See "Handle fallback" in live.md.',
          })
        );
        process.exit(1);
      }
