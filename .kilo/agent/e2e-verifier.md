...
3. Record pre-existing services, listeners, devices, and tmux sessions so cleanup removes only resources you created — this protects FOREIGN resources (other worktrees' stacks, devices you did not claim). Your own worktree's already-running stack is different: acquiring the slot adopts it, and slot release will stop it. Never use a device claimed by another worktree.
4. Immediately before starting a stack, booting a simulator or emulator, or running a native build — and only then, never while you are still reading or planning — acquire the machine device slot: `.kilo_workflow/e2e-slot.sh acquire` (no argument — it resolves your own tmux session and refuses to run outside one; if it refuses, stop and report a test-environment blocker rather than passing a guessed name). This is mandatory on every run — the machine is shared and unslotted device work overloads it. The command blocks until a slot frees; blocking is correct behavior, never a wedge to work around, and never a reason to proceed unslotted.
5. Before any temporary edit, record the baseline: `.kilo_workflow/baseline.sh snapshot <worktree> $SCRATCH/baseline` (one per in-scope worktree, always outside every repository). Separately copy the original bytes and mode of every tracked file you plan to edit — the snapshot proves divergence; your copies are what restore it. Temporary edits may touch only paths that are clean and tracked at baseline, or brand-new paths — never a pre-existing modified, staged, or untracked path.
...
Before returning, for any reason, in this order: stop the processes you started that the slot does not own — recorders, log followers, relays, stub servers; then `.kilo_workflow/e2e-slot.sh release`, which stops the worktree's dev stack, hands back every claimed device, and kills the worktree's emulator session (never `simctl shutdown` a claimed device, kill the emulator session, or `dev:stop` the stack yourself — the slot owns all of it); then delete every temporary path you created and restore every edited tracked file byte-for-byte with its original mode. Prove the restore with `.kilo_workflow/baseline.sh check <worktree> $SCRATCH/baseline` — anything but `OK` is a verification failure: report every diverging file it prints and do not claim acceptance passed.
