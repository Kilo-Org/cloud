...
2. Translate the plan's goals and acceptance criteria into observable flows; for user-facing features, cover the happy, retryable-unhappy, non-retryable-unhappy, and empty states. Batch aggressively: one booted device, one running stack, and one logged-in app session can verify MANY criteria — plan a route through the app that covers every reachable criterion in sequence and only relaunch, re-login, or reboot when a criterion genuinely requires fresh state. Separate runs per scenario burn device time and slot hours for nothing. For Appium, batch verifier-written flows into as few `apps/mobile/e2e/appium.sh <device> test … <flow.js> [more-flows.js]` invocations as possible (one WebDriver session per invocation; session startup dominates cost). `login.sh` / `logout.sh` keep their own per-call sessions.
3. Confirm the handoff names the live slot-bundle owner, worktree services, your assigned devices, and the platform scope (`ios` default; `android` or dual only with the recorded rationale). Verify only within that scope. Stop immediately with `VERIFICATION BLOCKED.` if the handoff omits ownership, an assigned resource is not ready, or the handoff's scope/devices disagree with what is running. The planner owns a repro bundle; the orchestrator owns a verification bundle. Never take another slot or start, stop, or replace its resources.
...
6. When `$E2E_ARTIFACTS` is set (section scratch `e2e-artifacts/`), evaluate any prior reusable flow scripts/assertions named in the handoff (`<platform>-<case>.js`) before rewriting them. Prior artifacts are evidence, not truth — reject faulty ones and re-derive. Save new reusable flows there under the handoff-assigned names. Baselines and temp files stay in the unique per-round `$SCRATCH`.
...
- **Video-first evidence:** record each flow segment (or criterion group) with `apps/mobile/e2e/record.sh <device> start <path>` / `stop`; extract report frames with `record.sh frame <video> <hh:mm:ss> <out.png>`. Segment recordings — never one whole-route video (Android `screenrecord` caps at ~3 minutes). Screenshots are for ad-hoc stills and the fallback when `record.sh` fails finalization; still return a verdict. Call `record.sh <device> stop` during your cleanup (idempotent).
- Capture concise evidence: segment videos, extracted frames, exact visible state, and bounded log excerpts. Never credentials.
...
- What was reused from `$E2E_ARTIFACTS` vs re-derived
