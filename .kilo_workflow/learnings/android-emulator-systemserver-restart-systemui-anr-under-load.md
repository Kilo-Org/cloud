...
**Symptom chain:** mid-run wrapped ADB `shell input tap` fails with `Broken pipe (32)` then `Can't find service: input`;
...
**Recovery (confirmed):** `pnpm dev:mobile:android adb -s <serial> root` works on the kilo AVDs
