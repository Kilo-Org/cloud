...
1. `uiautomator dump` bounds can describe a view the picker has already left (a tap aimed at them hit the wrong
   ...
   the same node's bounds with `apps/mobile/e2e/appium.sh <serial> hierarchy` — only tap when
   both agree. Prefer driver flows for navigation and reserve ADB taps for the final pick when sub-second screenshot timing matters.
2. `sonner-native` toasts are catchable deterministically with a wrapped ADB tap immediately followed by burst
   screencaps at +0.5s/+1.2s/+2.2s/+3.5s (`pnpm dev:mobile:android adb -s <serial> exec-out screencap -p > f.png`). A driver flow's
   session latency (seconds) is too slow — the toast dismisses before the shell regains control. The first
