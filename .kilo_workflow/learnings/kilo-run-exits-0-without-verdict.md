...
Fix: a round that produced no explicit verdict line is void, never a pass. Detect completion by requiring the verdict text itself (`No findings.` or a numbered findings list), never by exit code alone. Discard the void round and dispatch a fresh session — review gates want a fresh session per round anyway. Monitor the log for byte-size stagnation as well as process exit so a stall is distinguishable from work in progress. If several consecutive rounds die at the same point, shrink the handoff rather than retrying unchanged.
