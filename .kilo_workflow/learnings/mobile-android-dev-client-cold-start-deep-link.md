...
(a known state not visible within the launch budget) and never reaches
...
this machine, and 1–2 minutes under parallel-workflow load. `settle-app.js`'s first
`waitVisible(S.ANY_STATE)` budget is 15s; `open-app.js` gives Android cold launches 420s and
answers `Wait` on ANR dialogs, and login.sh's one retry goes through `open-app.js` — but any
hand-rolled probe with a short budget still fails during the reload. The retry path
cold-relaunches (`stopApp`/`launchApp`), restarting the clock. During the reload the screen is
uniform white and uiautomator dumps show only the status-bar clock.
...
- When `login.sh` fails but the app has since settled (probe with a scratch JS flow that waits
  ~20s for `you@example.com`), drive sign-in from a scratch JS flow instead of re-running it:
  `h.tapOn('you@example.com')` → `h.eraseText()` → `h.inputText(EMAIL)` →
  `h.tapOn('Send sign-in code')`; read the OTP from `dev/logs/emails`, ignoring the 6-digit
  number in the postal footer; handle `Accept and continue` and the notifications prompt (or
  reuse `flows/login-request-code.js` / `flows/login-verify-code.js` with `-e`). Preflight's
  effects (claim check, adb reverses, Metro provenance) survive the failed runs.
