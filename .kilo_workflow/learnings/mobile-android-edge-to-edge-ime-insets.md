...
is app-side: RN's `KeyboardAvoidingView` is inert on Android (the window frame never changes —
login-screen.tsx now enables KAV only on iOS, `behavior="padding"`), and the only thing that
moves content is JS consuming `Keyboard` events — currently a `paddingBottom:
androidKeyboardPad` wrapper (`androidKeyboardHeight + insets.bottom`, via
`resolveAppAwareKeyboardPadding`); under the number pad that padding grows the layout and the
`justify-center` container re-centers by half the pad (measured 373px for a 268dp IME in the
r0 build's `keyboardHeight + 16` spacer form). `keyboardDidShow` DOES fire with a real height
on Android for the number pad (268dp = IME height above the nav bar); a listener that
attaches while the keyboard is already up observes 0 and wrongly concludes the padding is inert.
