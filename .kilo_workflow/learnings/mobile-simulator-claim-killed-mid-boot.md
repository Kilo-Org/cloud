...
Symptom: `.kilo_workflow/e2e-start-resource.sh ios` is interrupted (e.g. the invoking shell's own
...
`.kilo_workflow/e2e-start-resource.sh ios` returns instantly with `alreadyOwned: true` and does
...
Prevention: run `.kilo_workflow/e2e-start-resource.sh ios` with a generous timeout (>= 10 min) —
