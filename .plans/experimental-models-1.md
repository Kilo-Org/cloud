...
- The opt-in copy for each preview model must disclose that prompts may be retained for experiment analysis and partner evaluation for the configured retention window, and that users are responsible for not submitting PII, secrets, customer data, or other sensitive content they do not want retained under that experiment policy. v1 must not run a real partner experiment until that model-specific opt-in/disclosure exists.
