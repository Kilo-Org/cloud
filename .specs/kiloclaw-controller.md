...
17. Agent read summaries MUST include the agent's routing bindings as reported by non-interactive OpenClaw CLI binding behavior (the routing source of truth), surfacing channel, account id (verbatim; absent account → null), and a flag for bindings more specific than a plain channel(/account) route (e.g. peer/guild/team/roles matches). The CLI binding listing covers routing bindings only; non-route binding types are out of scope for this surface and need not be reported.
...
19. The binding set MUST manage only channel-level default-account routes; account-scoped routes, advanced bindings (peer/guild/team/roles or non-`route` type), and other agents' bindings MUST be left intact. After a bind it MUST verify, against the CLI's own post-bind view, that every route the invocation produced is a channel-level default route; if a bind instead resolves to a route outside that scope (e.g. an account-scoped route), the set MUST roll back only the routes that invocation created and fail closed with `422 invalid_agent_config`.
20. The binding set MUST report a requested channel that the CLI rejects as already routed to another agent as `409 agent_binding_conflict`. Because OpenClaw applies non-conflicting additions before reporting a conflict, the binding set MUST roll back any routes it added and MUST NOT remove any existing route when a conflict occurs, so a rejected request leaves the agent's routing unchanged.
