...
| **Cost Insights** | Removed. No owned behaviour, code, or storage remains | `apps/web/src/components/cost-insights/CostInsightsDiscontinuedNotice.tsx` | Only the discontinued-notice routes remain |
...
- **Cost Insights is fully removed.** The UI, tRPC procedures, cron sweeps, spend capture, evaluation, alerting, rollup maintenance, and the `cost_insight_*` tables are all gone, and none may be reintroduced.
