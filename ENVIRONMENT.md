...
- `SLACK_ENCRYPTION_KEY` - Encrypts the Slack bot token at rest (AES-256-GCM) in the Chat SDK state store. Generate with `openssl rand -base64 32`; must decode to exactly 32 bytes or `apps/web` fails at startup. Reads stay backward compatible with existing plaintext entries, but once an entry is encrypted an environment without this key can no longer use it, so the variable must be present everywhere that shares a `REDIS_URL`. `[SECRET]`
