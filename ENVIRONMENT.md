...
- `GITHUB_AGENT_ONLY_CONNECTIONS_ENABLED` - Default-off rollout gate for new restricted secondary connections, in addition to the destination allowlist. Enable only after the role migration, web, git-token-service, Cloud Agent, Security Sync, and Security Auto Analysis are deployed and validated. The original association retains workflow authority; secondaries support Slack and Cloud Agent only. Disabling admission does not revoke existing sessions or promote surviving associations. Do not roll Workers back to role-unaware authorization after enabling admission. [SERVER]
