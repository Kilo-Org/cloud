...
- `NATIVE_RESOURCE_TOKENS_ENABLED` - Set to exact `true` to issue separate one-hour API/gateway access tokens only to native clients explicitly requesting `api-gateway-v1`. Default-off and independent of bounded internal assertions. Unsupported clients keep legacy responses. Enable only after reader deployment and compatible client routing/storage are verified; mobile downgrade can require re-login. [SERVER]
- `SHARED_RESOURCE_TOKENS_ENABLED` - WIP Phase 5.2 shared/control/runtime issuer migration; exact `true` opts in, unset/other values keep legacy issuance. Do not enable: runtime delivery and live credential renewal are incomplete, and modern Cloud Agent dispatch is intentionally fail-closed. The corresponding Worker vars remain `false`; no external rollout has been performed. [SERVER]
