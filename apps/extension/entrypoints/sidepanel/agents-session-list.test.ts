...
describe("query key functions", () => {
  describe("activeSessionsQueryKey", () => {
    it("produces the expected key for personal (null org)", () => {
      expect(activeSessionsQueryKey(null)).toEqual(["agents", "active-sessions", null]);
    });

    it("produces the expected key for an organization", () => {
      expect(activeSessionsQueryKey("org-123")).toEqual(["agents", "active-sessions", "org-123"]);
    });
  });

  describe("sessionHistoryQueryKey", () => {
    it("produces the expected key for personal (null org)", () => {
      expect(sessionHistoryQueryKey(null)).toEqual(["agents", "session-history", null]);
    });

    it("produces the expected key for an organization", () => {
      expect(sessionHistoryQueryKey("org-456")).toEqual(["agents", "session-history", "org-456"]);
    });
  });

  describe("sessionSearchQueryKey", () => {
    it("produces the expected key for personal (null org) with a query string", () => {
      expect(sessionSearchQueryKey(null, "test query")).toEqual([
        "agents",
        "session-search",
        null,
        "test query",
    ...
    it("produces the expected key for an organization with a query string", () => {
      expect(sessionSearchQueryKey("org-789", "search term")).toEqual([
        "agents",
        "session-search",
        "org-789",
        "search term",
