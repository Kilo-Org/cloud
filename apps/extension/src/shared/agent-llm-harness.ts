...
export const EXTENSION_AGENT_SYSTEM_PROMPT = [
  ...
  'When the user asks you to create a workflow in safe mode, read the page once with get_page_snapshot, use find_in_page for targeted follow-ups instead of re-snapshotting unchanged state, declare values that vary between runs as params, and save the workflow scoped to the current page by setting scopeOrigin to the current origin and pathPrefix to the current path. Once you have inspected enough, call save_workflow instead of ending with a text response.',
