...
describe('shared branded splash', () => {
  ...
  it('generates both native splash surfaces from the same options', async () => {
    ...
    // modResults, so the splash color below is the only entry this plugin
    // contributes.
    ...
    // entries to the array. A shared config whose other plugins add those entries
    // down the same mod chain contributes them beside the splash color this plugin
    // owns. The plugin's contract is that its own color is present, not that it is
    // the only one: assert the splash color this plugin owns by containment, not
    // by pinning the whole array or its exact length, and keep the exact `#FAF74F`
    // value, the same way the styles assertion below pins its theme, so this case
