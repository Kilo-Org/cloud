...
describe('shared branded splash', () => {
  ...
  it('generates both native splash surfaces from the same options', async () => {
    ...
    // that throwaway root again — and introspection seeds `android.colors` from
    // the `colors.xml` that root carries, falling back to empty `resources` when
    // the file is absent, so the project's own gitignored
    ...
    // present, not that it is the only one: a compile root that does carry a
    // `colors.xml`, or a shared config whose other plugins add icon, notification
    // and app-background entries down the same mod chain, contributes its entries
    // beside the splash color this plugin owns. Assert the splash color this
    // plugin owns by containment, not by pinning the whole array or its exact
    // length, and keep the exact `#FAF74F` value, the same way the styles
    // assertion below pins its theme, so this case speaks only for the entry this
    // plugin writes.
