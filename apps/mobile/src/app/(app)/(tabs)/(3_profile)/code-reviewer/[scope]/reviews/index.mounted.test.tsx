...
import CodeReviewerReviewListRoute from './index';
...
vi.mock('@/components/code-reviewer/review-list-screen', () => ({
  ReviewListScreen: 'ReviewListScreen',
}));
...
function mountRoute(): TestRenderer.ReactTestRenderer {
  ...
  act(() => {
    ref.current = TestRenderer.create(createElement(CodeReviewerReviewListRoute));
  });
  if (!ref.current) {
...
describe('CodeReviewerReviewListRoute invalid scope', () => {
  it('renders InvalidRouteState with the profile backTo when scope is undefined', () => {
    ...
    expect(findByType(renderer.root, 'ReviewListScreen')).toHaveLength(0);
...
describe('CodeReviewerReviewListRoute valid scope', () => {
  it('renders the review list with the parsed scope', () => {
    useLocalSearchParamsMock.mockReturnValue({ scope: 'personal' });
    const renderer = mountRoute();

    const screen = findByType(renderer.root, 'ReviewListScreen');
