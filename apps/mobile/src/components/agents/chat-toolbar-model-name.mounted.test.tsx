...
const DEFAULT_TOOLBAR_MODEL = 'deepseek/deepseek-v4.1-flash';
...
// Two call styles are supported: the positional `(model, modelOptions,
// variant)` form for the CLI-catalog cases and a props-override object for the
// loading-chip case.
function renderToolbar(
  modelOrOverrides: string | Partial<ComponentProps<typeof ChatToolbar>> = DEFAULT_TOOLBAR_MODEL,
  modelOptions: SessionModelOption[] = MODEL_OPTIONS,
  variant = 'low'
): TestRenderer.ReactTestRenderer {
  const overrides = typeof modelOrOverrides === 'string' ? {} : modelOrOverrides;
  const model = typeof modelOrOverrides === 'string' ? modelOrOverrides : DEFAULT_TOOLBAR_MODEL;
...
describe('ChatToolbar model chip while the model list loads', () => {
  it('keeps the chip shell with its label instead of a blank pill', () => {
    ...
    expect(chipLabel[0]?.props.numberOfLines).toBe(1);
    ...
    // The shell carries the busy state and the label for a screen reader.
    ...
    expect(shells[0]?.props.accessible).toBe(true);
    expect(shells[0]?.props.accessibilityLabel).toBe(i18n.t('common.model'));
  });
});
