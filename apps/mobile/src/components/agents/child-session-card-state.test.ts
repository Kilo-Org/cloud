...
describe('getChildSessionCardState', () => {
  ...
  it('marks a prompt-derived label as translatable and the fallback label as not', () => {
    const promptState = getChildSessionCardState(
      makeTaskPart('pending', { prompt: 'Review the access rules' }),
      []
    );
    expect(promptState).toMatchObject({
      taskName: 'Review the access rules',
      translatable: true,
    });

    const fallbackState = getChildSessionCardState(makeTaskPart('pending'), []);
    expect(fallbackState).toMatchObject({ taskName: 'Task', translatable: false });
  });
