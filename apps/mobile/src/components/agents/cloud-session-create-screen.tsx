...
export function CloudSessionCreateScreen() {
  ...
  const repositories = useMemo(() => {
    ...
    return (repoData.repositories as { fullName: string; private: boolean }[]).map(repository => ({
  ...
  const handleModelSelect = useCallback(
    (modelId: string, nextVariant: string) => {
      setModel(modelId);
      setVariant(nextVariant);
      saveModel(organizationId, { model: modelId, variant: nextVariant });
