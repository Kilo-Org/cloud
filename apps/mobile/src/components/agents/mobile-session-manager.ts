...
export function createMobileAgentSessionManager({
  ...
  return createSessionManager({
    ...
    userWebConnection,
    // A phone routinely drops the network mid-open: retry the failed open when
    ...
    // The mobile adapter is the canonical page provider. Nothing is persisted
    // from it: a session open must never paint a previous transcript, so the
    // skeleton stays up until this live page (or the transport replay) lands.
