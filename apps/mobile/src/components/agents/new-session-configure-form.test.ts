...
// The loading profile row renders the shimmer Skeleton, which reaches
// react-native-reanimated (and through it react-native-worklets). Both are
// native ESM packages the pure node project cannot load, so the host stand-in
// keeps the row's shape without the animation stack — the same mock the
// mounted profile-row test uses.
