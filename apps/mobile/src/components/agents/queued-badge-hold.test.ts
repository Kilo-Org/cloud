...
describe('nextHeldQueuedIds', () => {
  it('adds queued ids while streaming', () => {
    ...
    const pending = new Map<string, MessageDeliveryState>([['m1', { status: 'queued' }]]);
  ...
  it('returns the shared empty set when isStreaming is false', () => {
    ...
    const pending = new Map<string, MessageDeliveryState>([['m3', { status: 'queued' }]]);
