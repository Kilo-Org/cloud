...
describe('session detail header right cluster', () => {
  it('caps the right cluster inside the header slot and renders no copy control', async () => {
    ...
    // past the slot edge. It holds the context pill, nothing else: the PR badge
    // moved onto the goal row, so the header row carries no badge.
    ...
    expect(cluster?.findAllByType(SessionPrBadge)).toHaveLength(0);
