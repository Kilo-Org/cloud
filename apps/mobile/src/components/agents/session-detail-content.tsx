...
export function SessionDetailContent({
    ...
    async (text: string, options?: ChatComposerSendOptions) => {
      ...
      if (kind === 'remote-capable' && submission) {
        ...
        if (!result.ok) {
          ...
          // the SDK's composer status line never sees it. Surface the retryable
          // message through the toast channel and throw so the composer keeps
          // the draft/attachments for a retry.
      ...
      // rejecting — the SDK states the failure on the composer's status line,
      // which is the single surface for a failed send. Throw here, without a
      // second message, purely so the composer's `await onSend(...)` sees the
  ...
  const handleSendCommand = useCallback(
    async (command: string, argumentsText: string) => {
      ...
      // resolves the active remoteModelOverride from its own store; the SDK's
      // status line owns a failed send, so we throw a stable error on a false
      // return purely so the composer preserves the draft, and never emit a
      // duplicate message of our own.
      takeOverTranscriptPositionForSend();
      const sent = await manager.send({
  ...
  // composer's slash commands. The SDK's status line is the sole surface for a
  // failed send, so this helper never adds a message of its own. Edit throws
  // instead, so the RenameModal shows the failure inline and the user can
  // correct the objective. One helper keeps the `/goal` payload shape in one
  // place.
