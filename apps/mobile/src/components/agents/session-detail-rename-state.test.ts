...
describe('getSessionDetailRenameState', () => {
  ...
  it('normalizes a placeholder fallback title to the generic label', () => {
    // The helper runs both the server title and the caller fallback through the
    // shared title helpers, so a placeholder cached as the fallback cannot paint
    // a raw timestamp either. The screen pre-sanitizes the cached list title
    // before it becomes `fallbackTitle` (session-detail-content.tsx), so this is
    // the helper's own second line of defence.
