...
describe('SessionListHeaderActions new-session control', () => {
  ...
  it('meets the filter control at the row gap instead of overlapping its touch region', async () => {
    ...
    // The filter control carries a symmetric numeric `hitSlop` (its own
    // `h-11 w-11` frame plus 3pt per side), so both facing sides are that
    // number; the new-session control carries per-side values.
