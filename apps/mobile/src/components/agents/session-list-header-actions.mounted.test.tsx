...
/** One side's reach in dp: a numeric hitSlop widens every side by that many dp. */
function sideSlop(hitSlop: unknown, side: keyof Insets): number {
  if (typeof hitSlop === 'number') {
    return hitSlop;
  }
  return (hitSlop as Insets | undefined)?.[side] ?? 0;
}
...
describe('SessionListHeaderActions new-session control', () => {
  ...
  it('meets the filter control at the row gap instead of overlapping its touch region', async () => {
    ...
    const newSessionSlop = newSession.props.hitSlop;
    const filterSlop = filter.props.hitSlop;
    ...
    // The filter expresses its slop as one dp value for every side, while the
    // new-session control caps its right side, so read each side through
    // `sideSlop` rather than assuming both are per-side objects.
    expect(sideSlop(newSessionSlop, 'right') + sideSlop(filterSlop, 'left')).toBeLessThanOrEqual(
      gapDp
    );
    ...
    expect(
      box.width + sideSlop(newSessionSlop, 'left') + sideSlop(newSessionSlop, 'right')
