...
describe('buildMessageActionSheetOptions', () => {
  it('offers first-reaction choices for messages with no reactions', () => {
    const options = buildMessageActionSheetOptions({ isOwnMessage: false, canReact: true });
  ...
  it('offers edit and delete actions only for own messages', () => {
    const ownOptions = buildMessageActionSheetOptions({ isOwnMessage: true, canReact: true });
    const otherOptions = buildMessageActionSheetOptions({ isOwnMessage: false, canReact: true });
