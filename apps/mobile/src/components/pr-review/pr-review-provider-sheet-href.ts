...
// composer, the conversation-comment composer, the review-submit sheet and
// the merge sheet are children of the PR's own route on every provider: the
// provider scope and the `PendingReviewProvider` queue are published by the
// provider layout, so a provider sheet must be reached through the provider
// route — pushing the GitHub sibling would leave that scope and write to the
// wrong provider. The GitHub route keeps its own literal paths untouched.
