...
export function PrReviewReconnectNotice() {
  ...
  const title =
    platform === 'gitlab'
      ? t('prReview.reconnectNotice.gitlabTitle')
      : platform === 'bitbucket'
        ? t('prReview.reconnectNotice.bitbucketTitle')
  ...
  const message =
    providerPlatform === null
      ...
      : t('prReview.reconnectNotice.providerMessage', {
          provider:
            platform === 'gitlab' ? t('common.gitlab') : t('agentChat.repoPicker.platformBitbucket'),
        });
