...
describe('Text tracked labels in RTL', () => {
  ...
  it('applies the same reset to the shared Eyebrow label', () => {
    ...
    // The eyebrow variant owns its Latin display classes and renders its own
    // tracked class in LTR only, so an RTL eyebrow carries neither the display
    // classes nor a tracked class of its own (`text.mounted.test.tsx`); the RTL
    // style reset is what keeps any tracked class a caller passes in harmless.
