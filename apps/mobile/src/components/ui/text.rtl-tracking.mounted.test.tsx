...
describe('Text tracked labels in RTL', () => {
  ...
  it('applies the same reset to the shared Eyebrow label', () => {
    ...
    // the variant owns its display classes, so an RTL eyebrow drops them (the
    // rule `text.mounted.test.tsx` pins) rather than keeping them like a
    // caller-supplied tracked class, and no uppercase/tracked class is left for
    // the shared reset to neutralize on this label; the zero letter-spacing
    // reset still lands.
