 ...
 * (`apps/web/src/routers/cloud-agent-next-schemas.ts`). A host that is not a
 * known provider is null (unknown); a bare path with no host defaults to
 * GitHub, whose repository form is exactly `owner/repo` — a GitLab project
 * with subgroups is addressed by its URL instead.
 */
