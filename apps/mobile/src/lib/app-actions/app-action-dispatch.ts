...
export async function dispatchAppActionRequest(
  ...
  if (typeof href !== 'string') {
    // In practice only `OpenPullRequest` reaches here: the contract's builders
    // return string paths, and a link the provider resolvers do not recognize
    // resolves to nothing. Re-running cannot change that.
    return {
      ok: false,
