...
//
// The shared core lives in `prepare-agent-session.ts` and is re-exported here;
// this module resolves what only the action path has to resolve for itself —
...
export { prepareAgentSession };
export type {
  PrepareAgentSessionDeps,
  PrepareAgentSessionInput,
  PrepareAgentSessionOutcome,
  SafeRetryRowInput,
};
...
export async function startAgent(input: StartAgentInput): Promise<AppActionResult> {
  ...
  try {
    ...
    if (sessionId === null && input.prompt.trim().length === 0) {
      return startAgentFailure('empty-prompt', false, failedToCreate);
    }
