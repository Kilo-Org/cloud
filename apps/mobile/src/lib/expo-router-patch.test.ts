...
const patch = readFileSync(
  new URL('../../../../patches/expo-router@57.0.22.patch', import.meta.url),
  'utf8'
);
...
describe('Expo Router provider review routes', () => {
  ...
  it('locks every router snapshot to the current patch bytes', () => {
    ...
    expect(lock).toContain(`expo-router@57.0.22: ${hash}`);
    const hashes = [
      ...lock.matchAll(/(?:expo-router(?:@|: )|version: )57\.0\.22\(patch_hash=([a-f0-9]+)\)/g),
    ].map(match => match[1]);
