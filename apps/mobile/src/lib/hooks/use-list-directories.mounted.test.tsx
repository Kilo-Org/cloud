...
describe('useListDirectories', () => {
  ...
  it('resets a transport failure to the skeleton on retry, then recovers', async () => {
    ...
    listFn.mockReturnValueOnce(retry);
    act(() => {
      api().list('src');
    });
  ...
  it('nudges a disconnected transport before refetching on Retry', async () => {
    // The reconnect loop may have parked on a long backoff or exhausted itself
    // while the sheet sat on the error; Retry must restart it, not only refetch
    // into a socket nobody is rebuilding. A connected socket is left alone.
    listFn.mockResolvedValueOnce({ ok: false, reason: 'transport' });
    const { api, unmount } = await mount('conn-1');
