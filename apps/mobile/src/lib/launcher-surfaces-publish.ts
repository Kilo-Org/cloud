 ...
 * a label for a shortcut that exists. "Needs input" and "New agent" reuse
 * `glanceable.needsInput` and `glanceable.newAgent`: the widget exposes the
 * same two actions, so one key per action keeps the labels from drifting, and
 * a second key holding either copy fails the catalog duplicate check.
 */
