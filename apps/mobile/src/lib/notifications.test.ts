...
describe('Android Do Not Disturb override reconciliation', () => {
  ...
  it('never deletes a channel to apply the override, which would drop the ongoing card', async () => {
    // Measured on API 36: the framework applies the access gate only while it
    // creates a channel, and a delete plus recreate un-deletes the channel with
    // its previous settings, so a recreate drops the posted card and changes
    // nothing else.
    mocks.requireOptionalNativeModule.mockReturnValue({
      isAgentProgressAllowed: () => true,
      isDndAccessGranted: () => false,
    });
    mocks.getNotificationChannelAsync.mockResolvedValue({
      id: 'needs-input',
      bypassDnd: true,
    });
    const { renameAndroidNotificationChannels } = await loadNotifications();
