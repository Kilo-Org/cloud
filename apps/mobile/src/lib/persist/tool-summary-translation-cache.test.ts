...
describe('tool summary translation cache', () => {
  ...
  it('hydrates a full cache without per-entry reads', async () => {
  ...
  it.each([{ language: 'fr' }, { modelId: 'kilo-auto/large' }])(
    'keeps entries apart when only %j differs',
    async variant => {
      const original = makeEntry();
      const other = makeEntry(variant);
      await writeToolSummaryTranslation(original);
      await writeToolSummaryTranslation(other);

      await expect(readToolSummaryTranslations()).resolves.toEqual([original, other]);
