...
describe('recent-prs', () => {
  ...
  it('upsert keeps one row per provider for a same-named repo', async () => {
    ...
    await upsertRecentPr(
      makeRecent({ number: 1, title: 'GitLab one', platform: 'gitlab', instanceHint: 'https://gitlab.example.com' })
    );
  ...
  it('upsert dedupes a re-opened provider entry by platform and instance', async () => {
    await upsertRecentPr(
      makeRecent({ number: 1, title: 'Old', platform: 'gitlab', instanceHint: 'https://gitlab.example.com' })
    );
    await upsertRecentPr(
      makeRecent({ number: 1, title: 'New', platform: 'gitlab', instanceHint: 'https://gitlab.example.com' })
    );
    await upsertRecentPr(
      makeRecent({ number: 1, title: 'Other instance', platform: 'gitlab', instanceHint: 'https://gitlab.other' })
    );
  ...
  it('remove and mark-failed target one provider row, not its twins', async () => {
    await upsertRecentPr(makeRecent({ number: 1, title: 'GitHub one' }));
    await upsertRecentPr(
      makeRecent({ number: 1, title: 'GitLab one', platform: 'gitlab', instanceHint: 'https://gl.acme.dev' })
    );

    await removeRecentPr({ owner: 'octocat', repo: 'hello-world', number: 1, platform: 'gitlab', instanceHint: 'https://gl.acme.dev' });
  ...
  it('recentPrKey is collision-free across providers and folds case for GitHub', () => {
    ...
    expect(
      recentPrKey({ ...triple, platform: 'gitlab', instanceHint: 'https://A.example/' })
    ).toBe(recentPrKey({ ...triple, platform: 'gitlab', instanceHint: 'http://a.example' }));
  });
