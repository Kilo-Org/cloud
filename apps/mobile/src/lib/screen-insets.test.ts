...
describe('screen side insets: one implementation for both platforms', () => {
  ...
  it('has the Profile screen read its side insets from the entry point', () => {
    ...
    const withoutSignOutConfirm = profile.replace(
      /const confirmSignOut = \(\) => \{[\s\S]*?\n {2}\};/,
      ''
    );
    expect(
      withoutSignOutConfirm,
      `${PROFILE_SCREEN} forks on the platform outside the sign-out confirmation`
