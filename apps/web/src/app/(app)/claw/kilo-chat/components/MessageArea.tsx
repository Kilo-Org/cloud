...
export function MessageArea({ conversationId }: MessageAreaProps) {
  ...
  const currentUserLoaded = currentUserId !== null;
  const botCanReceive = botDisplay.state === 'online' || botDisplay.state === 'idle';
  const canSend = currentUserLoaded && botCanReceive;
          ...
          <div ref={contentRef} className="flex flex-1 flex-col">
            ...
            {messages.map(msg => (
              <MessageBubble
                ...
                userActionsEnabled={currentUserLoaded}
              />
            ))}
          </div>
        </div>
