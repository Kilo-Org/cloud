...
function formatRuntimeLabel(version: string | null, releasedAt: string | null): string {
  if (!version && !releasedAt) return 'Unknown';
  const date = releasedAt
    ? new Date(`${releasedAt}T00:00:00.000Z`).toLocaleDateString('en-GB', {
        day: 'numeric',
        month: 'short',
        year: 'numeric',
        timeZone: 'UTC',
...
export function VercelComputeSettings({ organizationId }: { organizationId: string }) {
  ...
  const statusQuery = useQuery({
    ...
    refetchInterval: query =>
      ...
      query.state.data?.upgradeStatus === 'pending' ||
      query.state.data?.upgradeStatus === 'building'
        ? 2_000
        : false,
  });
  ...
  const isUpgradeRunning =
    status?.upgradeStatus === 'pending' || status?.upgradeStatus === 'building';
  const setupStatus = status
    ? isUpgradeRunning
      ? { label: 'Upgrading', variant: 'secondary' as const }
      : statusCopy(status.setupStatus, enrollmentState)
    : null;
  const progressStep = isUpgradeRunning ? status?.upgradeStep : status?.setupStep;
  const currentStep = progressStep ? SETUP_STEPS.findIndex(([step]) => step === progressStep) : -1;
              ...
              <form className="space-y-5" autoComplete="off" onSubmit={submit}>
                {!hasTeams ? (
                  ...
                  <>
                    ...
                    <ul
                      id="vercel-setup-notice"
                      className="text-muted-foreground list-disc space-y-1 border-t pt-4 pl-4 text-xs leading-relaxed"
                    >
                      <li>Vercel bills your team for setup and sessions.</li>
                      <li>Setup does a one-time build in your Vercel project.</li>
                      <li>Your Vercel token is stored encrypted when you start setup.</li>
                    </ul>
                  </>
                )}
                {setupError && (
          ...
          <>
                  ...
                  <div className="space-y-4">
                    <div className="bg-muted/40 flex flex-col gap-3 rounded-lg border p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        ...
                        <p className="text-muted-foreground text-xs">
                          Version{' '}
                          {formatRuntimeLabel(
                            status.runtimeWrapperVersion,
                            status.runtimeReleasedAt
                          )}
                        </p>
                    ...
                    {isUpgradeRunning && (
                      <div className="space-y-3">
                        ...
                        <p className="text-muted-foreground text-xs">
                          Open sessions keep the current snapshot. New sessions use it until this
                          upgrade finishes.
                        </p>
                      </div>
                    )}
                    {status.upgradeStatus === 'failed' && (
                      <Alert variant="destructive">
                        <AlertTriangle />
                        <AlertTitle>Runtime upgrade could not finish</AlertTitle>
                        <AlertDescription>
                          The current snapshot is still in use. Check Vercel capacity and token
                          access, then retry the upgrade.
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
            ...
            {((status.setupStatus !== 'ready' && status.setupStatus !== 'failed') ||
              ...
              <p className="text-muted-foreground text-center text-xs">
                You can safely close this page. Setup continues in the background.
              </p>
            )}
          </>
        ) : null}
