...
export function ReviewAgentPageClient({
  ...
  const handleBitbucketViewChange = (view: string) => {
    ...
    router.push(`/organizations/${organizationRouteIdentifier}/code-reviews?${params.toString()}`);
  };
  ...
  const githubIntegrationPath = returnTo
    ? `/organizations/${organizationRouteIdentifier}/integrations/github?${new URLSearchParams({
        returnTo,
      })}`
    : `/organizations/${organizationRouteIdentifier}/integrations/github`;
