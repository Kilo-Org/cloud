...
export async function GET(
  request: NextRequest
): Promise<NextResponse<{ error: string } | { data: ControllerTelemetryRow[] }>> {
...
FORMAT JSON`;
  ...
  const typedRows: ControllerTelemetryRow[] = result.data as unknown as ControllerTelemetryRow[];
  return NextResponse.json({ data: typedRows });
}
