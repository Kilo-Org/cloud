...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (response.status === 402) {
      captureMessage('OpenRouter System One balance exhausted', { level: 'error' });
    ...
    captureException(error, { tags: { source: 'systemone_proxy' } });
  ...
  if (!result.success) {
    captureMessage('Invalid OpenRouter System One response or missing usage', { level: 'error' });
