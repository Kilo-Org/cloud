...
jest.mock('@/lib/bot/platform-helpers', () => ({
  ...
  getPlatformIntegrationById: jest.fn(async () => {
    throw new MockPlatformIntegrationUnavailableError('disconnected');
  }),
}));
jest.mock('@/lib/bot', () => ({ bot: { initialize: mockBotInitialize } }));
jest.mock('@/lib/bot/platforms', () => ({ botPlatforms: {} }));
