  ...
  return (
    <div className="grid gap-3 p-5">
      ...
      <div className="divide-y overflow-hidden rounded-lg border">
        ...
        {/* Non-interactive: model access is the only policy type today, so the
            picker would otherwise look broken once it is configured. */}
        <div className="text-muted-foreground flex items-center gap-3 px-4 py-3">
          <Sparkles className="size-5 shrink-0" />
          <span className="min-w-0 flex-1">
            <span className="type-body block font-medium">More policies coming soon</span>
            <span className="type-label block">
              {available.length === 0
                ? 'Every policy type available today is already configured.'
                : 'Model access is the only policy type available today.'}
            </span>
          </span>
        </div>
      </div>
