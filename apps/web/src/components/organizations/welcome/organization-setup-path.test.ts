...
describe('organization setup paths', () => {
  it.each(['github', 'code-reviewer', 'invite-team', 'complete'] as const)(
    'parses the %s screen',
    screen => {
  ...
  it('selects the first incomplete screen and falls back to complete', () => {
    expect(getFirstIncompleteOnboardingScreen(checklist([false, true, false]))).toBe('github');
  ...
  it('builds canonical paths and transitions', () => {
    expect(buildOrganizationWelcomePath('org-id', 'github')).toBe(
      '/organizations/org-id/welcome?step=github'
    );
    expect(
    ...
    expect(getPreviousOnboardingScreen('github')).toBeNull();
