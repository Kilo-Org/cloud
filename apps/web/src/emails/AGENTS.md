...
| `clawInstanceReady.html`               | `claw_url`, `trial_period`, `year`. `trial_period` is a pre-formatted string (e.g. `"1 day"` or `"7 days"`) derived from the subscription's `kiloclaw_price_version` via the pricing catalog.                                                                                                           | —                          |
