...
async function getOpenRouterModelIds(redisGetFn: typeof redisGet = redisGet): Promise<Set<string>> {
  try {
    const raw = await redisGetFn(GATEWAY_METADATA_REDIS_KEYS.openrouterModels);
...
export async function getSuggestedFreeModels(
  redisGetFn: typeof redisGet = redisGet
): Promise<string[]> {
  const openrouterModelIds = await getOpenRouterModelIds(redisGetFn);
