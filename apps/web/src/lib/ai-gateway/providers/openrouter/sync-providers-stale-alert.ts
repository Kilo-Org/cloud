...
export function isLiveStaleSyncAlertDeliveryEnabled(
        ...
        elements: [
          {
            ...
            text:
              kind === 'test'
                ? `Test alert posted at ${input.now.toISOString()} · <${APP_URL}/admin/gateway|Open Gateway admin>`
...
export async function postStaleSyncAlert(input: {
  ...
  const sendNotification = input.sendNotification ?? sendAdminSlackNotification;
...
export async function alertIfSyncProvidersStale({
  ...
  sendNotification = sendAdminSlackNotification,
  isLiveDeliveryEnabled = isLiveStaleSyncAlertDeliveryEnabled,
}: StaleAlertDependencies = {}): Promise<void> {
  try {
    if (!isLiveDeliveryEnabled()) {
      console.info(
