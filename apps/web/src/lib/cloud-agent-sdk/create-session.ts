 ...
 *
 * `create_session` is a connection-scoped viewer command sent on the
 * user-web socket. Its success body is the strict `protocolVersion: 1`
 * envelope (see `createSessionResponseV1Schema`). Anything outside that
 * shape — extra fields, missing fields, the wrong protocol version, or a
 * non-string `sessionID` — is rejected; the relay is the source of truth
 * for this envelope and any drift must fail closed.
 *
