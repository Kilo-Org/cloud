...
describe('resolveCheapSameVendorSmallModel', () => {
  ...
  it('does not substitute a Kilo-billed catalog sibling for an uncatalogued BYOK primary', () => {
    const mixedCatalog = catalogPricesFromStoredModels({
      'neuralwatt/glm-tiny': priced('neuralwatt/glm-tiny', '0.0000001'),
    });
    expect(resolveCheapSameVendorSmallModel('neuralwatt/glm-5.2-short', mixedCatalog)).toBe(
      'neuralwatt/glm-5.2-short'
    );
  });
