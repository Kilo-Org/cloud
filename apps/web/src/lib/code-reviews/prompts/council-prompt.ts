...
export function buildCouncilOrchestratorPrompt(params: {
  ...
  const coordination = [
    ...
    '- Begin the review SUMMARY with a "## Council review" section: a markdown table with one',
    '  row per specialist — columns `Specialist | Model | Vote | Findings` — followed by the',
    '  overall decision on its own line (e.g. `**Decision:** ⚠️ Warn`). Render every vote and',
    '  the decision with these icons: ✅ Pass, ⚠️ Warn, ⛔ Block, ➖ Abstain. Derive the',
    `  decision by applying the governance rule above (${formatAggregationStrategy(aggregationStrategy)}).`,
    '',
    ...
    'field for a specialist that did not report one. Do NOT put an overall decision in the',
    'manifest — our system computes the authoritative decision from these votes (the decision',
    'you render in the PR summary above is for human display only).',
    '',
    '---',
    '',
