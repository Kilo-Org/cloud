...
const COPY_BY_TERMINAL_REASON = new Map<CodeReviewTerminalReason, CodeReviewTerminalReasonCopy>([
  [
    'assistant_no_actionable_output',
    {
      ...
      checkSummary:
        'The review session ended without producing a review summary or comments: the model exhausted its output limit while reasoning.',
      summaryBody: `${KILO_REVIEW_MARKER}
## Code Review Summary

**This review produced no output.** The model exhausted its output limit while
reasoning, so it finished without generating a review summary or inline
comments. Re-run the review (for example by pushing a new commit) to try again.`,
    },
  ],
  [
