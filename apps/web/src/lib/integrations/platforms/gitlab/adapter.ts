...
export async function fetchGitLabProjectDetails(
  ...
  const encodedProjectId =
    typeof projectId === 'string' ? encodeURIComponent(projectId) : projectId;
  const response = await fetchGitLab(
    buildGitLabUrl(instanceUrl, `/api/v4/projects/${encodedProjectId}`),
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
...
export async function createOrUpdateGitLabTextFile(
  ...
  const encodedProjectId =
    typeof projectId === 'string' ? encodeURIComponent(projectId) : projectId;
  const response = await fetchGitLab(
    buildGitLabUrl(instanceUrl, `/api/v4/projects/${encodedProjectId}/repository/commits`),
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export async function createGitLabMergeRequest(
  ...
  const encodedProjectId =
    typeof projectId === 'string' ? encodeURIComponent(projectId) : projectId;
  const response = await fetchGitLab(
    buildGitLabUrl(instanceUrl, `/api/v4/projects/${encodedProjectId}/merge_requests`),
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
