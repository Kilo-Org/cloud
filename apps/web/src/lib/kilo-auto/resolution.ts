...
export async function resolveAutoModel(
    ...
    return {
      model:
        (await balancePromise) > 0
          ? GPT_5_NANO_ID
          : gpt_oss_20b_free_model.public_id,
    };
  }
  ...
  if (model === KILO_AUTO_BALANCED_MODEL.id) {
    if (mode === modeSchema.enum.KiloClaw) {
      ...
      if (
        user &&
        (await userIsWithinFirstKiloClawInstanceWindow({ userId: user.id }))
      ) {
        return BALANCED_CLAW_SETUP_MODEL;
      }
    }
    return (
      (mode !== null ? BALANCED_MODE_TO_MODEL[mode] : null) ??
      BALANCED_CODE_MODEL
    );
  }
  return (
    (mode !== null ? FRONTIER_MODE_TO_MODEL[mode] : null) ?? FRONTIER_CODE_MODEL
  );
}
  ...
  if (resolved.verbosity) {
    if (request.kind === "messages") {
      ...
      request.body.text = {
        ...request.body.text,
        verbosity:
          resolved.verbosity as OpenAI.Responses.ResponseTextConfig["verbosity"],
      };
    } else {
