...
type KiloPassAffiliateSaleCandidate = {
  userId: string;
  tier: KiloPassTier;
...
function getKiloPassAffiliateSaleReportingFields(candidate: KiloPassAffiliateSaleCandidate) {
  const reportingFields = KILO_PASS_AFFILIATE_SALE_REPORTING[candidate.tier][candidate.cadence];
  return candidate.itemSku ? { ...reportingFields, itemSku: candidate.itemSku } : reportingFields;
}
...
export async function handleKiloPassInvoicePaid(params: {
  ...
  const affiliateSaleState: { candidate: KiloPassAffiliateSaleCandidate | null } = {
    candidate: null,
  };
  ...
  const affiliateSaleCandidate = affiliateSaleState.candidate;
  if (affiliateSaleCandidate !== null) {
    try {
      const eventDate =
      ...
      await enqueueAffiliateEventForUser({
        userId: affiliateSaleCandidate.userId,
        provider: 'impact',
        eventType: 'sale',
        ...
        ...getKiloPassAffiliateSaleReportingFields(affiliateSaleCandidate),
        ...(promoCode ? { promoCode } : {}),
      ...
      logWarning('Affiliate sale enqueue failed', {
        stripe_event_id: eventId,
        user_id: affiliateSaleCandidate.userId,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
