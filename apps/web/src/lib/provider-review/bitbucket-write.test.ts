...
jest.mock(
  "@/lib/integrations/platforms/bitbucket/workspace-access-token-repository-cache",
  () => ({
...
describe("addComment", () => {
  it("posts the raw content to the pull request comments collection", async () => {
    ...
    expect(post?.url.pathname).toBe(
      "/2.0/repositories/acme/repo/pullrequests/12/comments",
    );
  ...
  it("a startLine range spans from the first line to the anchor line", async () => {
    await addComment({
      owner: ORG_OWNER,
      workspace: "acme",
      repoSlug: "repo",
    ...
    expect(JSON.parse(String(post?.init.body))).toEqual({
      ...
      inline: { path: "src/deploy.ts", from: 10, to: 20 },
    });
  });
  ...
  it("maps a provider 403 to non-retryable forbidden", async () => {
    fetchMock.mockImplementation(async (url: string | URL) => {
      ...
      if (new URL(full).pathname === "/2.0/user")
...
describe("submitReview", () => {
  it("maps approve to the participants state approved for the connected identity", async () => {
    ...
    const put = bitbucketCalls().find(
      (call) =>
        ...
        call.url.pathname.includes("/participants/"),
    );
  ...
  it("posts every inline comment before the review state and the summary comment", async () => {
    ...
    expect(
      effects.map((call) => `${String(call.init.method)} ${call.url.pathname}`),
    ).toEqual([
    ...
    expect(JSON.parse(String(effects[1].init.body))).toEqual({
      ...
      // A startLine range spans from the first line to the anchor line.
      inline: { path: "b.ts", from: 4, to: 9 },
    });
  ...
  it("a mid-batch provider rejection surfaces the classified error and stops the batch", async () => {
    let posts = 0;
    fetchMock.mockImplementation(async (url: string | URL) => {
      const full = url.toString();
      if (full.includes("token-service.example.com")) {
      ...
      if (parsed.pathname.endsWith("/comments")) {
        ...
        return posts === 1
          ? jsonResponse({ id: 101 })
          : jsonResponse(
              { error: { message: "inline position invalid" } },
              400,
            );
      }
      return jsonResponse({});
    ...
    expect(bitbucketCalls().some((call) => call.init.method === "PUT")).toBe(
      false,
    );
    expect(posts).toBe(2);
  });
...
describe("resolveThread", () => {
  it("resolves the comment task when one exists", async () => {
    ...
    expect(put?.url.pathname).toBe(
      "/2.0/repositories/acme/repo/pullrequests/12/tasks/7",
    );
  ...
  it("follows the paginated task collection and resolves the task on a later page", async () => {
    ...
    expect(
      bitbucketCalls().filter((call) => call.url.pathname.endsWith("/tasks")),
    ).toHaveLength(2);
  });
...
describe("mergePullRequest", () => {
  it("re-fetches the PR, fences the head, and merges the exact revision", async () => {
    ...
    const post = bitbucketCalls().find((call) => call.init.method === "POST");
    expect(post?.url.pathname).toBe(
      "/2.0/repositories/acme/repo/pullrequests/12/merge",
    );
    expect(JSON.parse(String(post?.init.body))).toEqual({
  ...
  it("refuses a stale revision with the exact reason and never merges", async () => {
    ...
    expect(
      bitbucketCalls().some((call) => call.url.pathname.endsWith("/merge")),
    ).toBe(false);
  });
  ...
  it("reports replayed when the PR is already merged", async () => {
    ...
    expect(result).toEqual({ done: true, replayed: true });
    expect(bitbucketCalls().some((call) => call.init.method === "POST")).toBe(
      false,
    );
  });
...
describe("capabilities and reasons", () => {
  ...
  it("reactions are unsupported with the provider reason", () => {
    expect(BITBUCKET_PR_REVIEW_CAPABILITIES.reactions).toEqual({
      supported: false,
      reason:
        "Bitbucket Cloud does not expose reactions on pull request comments",
    });
  });
