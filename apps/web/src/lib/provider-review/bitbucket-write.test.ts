...
describe('submitReview', () => {
  it('maps approve to the participants state approved for the connected identity', async () => {
    ...
    // The participant id is the server-derived workspace uuid: a workspace
    // access token cannot resolve an account itself (/2.0/user answers 403).
    expect(put?.url.pathname).toBe(
      `/2.0/repositories/acme/repo/pullrequests/12/participants/${encodeURIComponent(WORKSPACE.uuid)}`
    );
    expect(JSON.parse(String(put?.init.body))).toEqual({ state: 'approved' });
    expect(bitbucketCalls().some(call => call.url.pathname === '/2.0/user')).toBe(false);
  });
  ...
  it('posts every inline comment before the review state and the summary comment', async () => {
    ...
    expect(effects.map(call => `${String(call.init.method)} ${call.url.pathname}`)).toEqual([
      ...
      'POST /2.0/repositories/acme/repo/pullrequests/12/comments',
      `PUT /2.0/repositories/acme/repo/pullrequests/12/participants/${encodeURIComponent(WORKSPACE.uuid)}`,
      'POST /2.0/repositories/acme/repo/pullrequests/12/comments',
    ]);
...
describe('unresolveThread', () => {
  it('reopens the resolved comment task with the provider state field', async () => {
    fetchMock.mockImplementation(async (url: string | URL) => {
      const full = url.toString();
      if (full.includes('token-service.example.com')) {
        return jsonResponse({
    ...
    const put = bitbucketCalls().find(call => call.init.method === 'PUT');
    expect(put?.url.pathname).toBe('/2.0/repositories/acme/repo/pullrequests/12/tasks/7');
    // Bitbucket reads `state`, not a boolean `resolved`: the body must name
    // the enum or the reopen is a silent no-op that still answers 200.
    expect(JSON.parse(String(put?.init.body))).toEqual({ state: 'UNRESOLVED' });
  });

  it('reports replayed when the task is already unresolved', async () => {
    fetchMock.mockImplementation(async (url: string | URL) => {
      const full = url.toString();
      if (full.includes('token-service.example.com')) {
        return jsonResponse({
