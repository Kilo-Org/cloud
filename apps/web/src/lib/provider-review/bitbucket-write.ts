...
/**
 * The account id Bitbucket attributes to this token's actions: the connected
 * workspace's uuid, resolved server-side by the authorization layer. A
 * workspace access token cannot resolve an account itself (`/2.0/user`
 * answers 403 — the same limit the inbox documents), so the server-derived
 * workspace uuid is the only participant id this layer can use.
 */
function ownAccountId(access: BitbucketRepositoryAccess): string {
  return access.workspace.uuid;
}

/**
 * The Bitbucket `inline` block for one anchor: RIGHT anchors the destination
 * line (`to`), LEFT the source line (`from`). `from`/`to` are source-side and
 * destination-side line numbers, not a one-sided range — sending
 * `from: startLine` for a RIGHT range would anchor an unrelated source line,
 * and a LEFT range would invent a new-side line. So a `startLine` range
 * anchors its end line on the tapped side; the range stays in the pending
 * list and the ledger key, not in the provider position.
 */
...
/**
 * The partial-apply reason: an inline comment committed before a later
 * rejection, so the provider already holds effects a replayed batch would
 * duplicate. The retryable kind keeps the router's ledger row
 * reconcile_pending instead of settling it failed.
 */
const BITBUCKET_INLINE_PARTIAL_APPLY_REASON =
  'Bitbucket applied part of this review before the request failed. Check the pull request before retrying.';
 ...
 * the summary comment, so a review carries GitHub-parity inline threads;
 * once any inline comment has committed, every failure reports the
 * retryable kind, so the router marks the ledger row reconcile_pending and
 * a same-key retry never re-posts the committed comments as duplicates.
 */
  ...
  try {
    for (const comment of target.comments ?? []) {
      try {
        ...
        // A rejection after an earlier comment committed is a partial
        // apply: the deterministic kind would settle the ledger row failed
        // and let a key-rotating retry re-post the committed comments.
        throw inlineCommitted
          ? new BitbucketReviewError('retryable', BITBUCKET_INLINE_PARTIAL_APPLY_REASON)
          : error;
      }
      inlineCommitted = true;
    }
    ...
    const classified = classifyBitbucketError(error);
    // Once an inline comment is live, every later failure — mid-batch or
    // the state/summary step — is a partial apply: reconcile, never settle
    // failed (see BITBUCKET_INLINE_PARTIAL_APPLY_REASON).
    if (inlineCommitted && !classified.retryable) {
      throw new BitbucketReviewError('retryable', BITBUCKET_INLINE_PARTIAL_APPLY_REASON);
    }
    throw classified;
  }
}
 ...
 * A head move visible before the merge call is refused BEFORE any merge
 * call, so a stale revision is never redirected into a merge. Bitbucket
 * Cloud exposes no server-side head precondition (see mergePullRequest), so
 * a push landing between this read and the merge POST itself is a
 * provider-level residual the preflight cannot close.
 */
 ...
 * against a fresh fetch immediately before the merge call, so a head the
 * reviewer did not see is refused, never redirected or merged. Bitbucket
 * Cloud's merge endpoint carries no head/revision precondition the request
 * itself can check (GitHub's `sha` parameter has no Bitbucket equivalent;
 * the documented request body is only type/message/close_source_branch/
 * merge_strategy), so a precondition field in the body would assert a server
 * check that does not exist. The flow stays enabled because Bitbucket
 * supports merging and this preflight is the strongest check it allows.
 * `closeSourceBranch` is honored.
 */
