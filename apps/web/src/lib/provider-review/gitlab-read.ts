...
export async function getMergeRequest(
  ...
  try {
    const [mr, headSha, diffRefs] = await Promise.all([
      fetchGitLabMergeRequest({
      ...
      getMRDiffRefs(access.accessToken, access.projectPath, mrIid, access.instanceUrl),
    ]);
    ...
    return {
      ...
      headSha: diffRefs?.headSha || headSha || detail.sha,
...
/** The pipelines running on the MR head commit, as the shared checks DTO. */
export async function listChecks(
  ...
  const access = await authorizeProject(owner, projectPath, instanceHint);
  try {
    const headSha = await getMRHeadCommit(
      access.accessToken,
      access.projectPath,
      mrIid,
      access.instanceUrl
    ...
    const pipelines = await requestGitLabJson<GitLabPipeline[]>(
      access,
      `/api/v4/projects/${projectSegment(access)}/pipelines`,
      { query: { sha: headSha, per_page: GITLAB_PAGE_SIZE } }
    );
    return {
...
export async function listInbox(
  ...
  try {
    ...
    const mergeRequests = await requestGitLabJson<GitLabMergeRequestDetail[]>(
      ...
      {
        // reviewer_username is supported on gitlab.com and on self-managed
        // GitLab since 10.x — older than the versions we advertise.
        query: {
          state: 'opened',
