...
jest.mock("@/lib/integrations/platforms/gitlab/adapter", () => ({
  ...
  fetchGitLabMergeRequest: (params: unknown) =>
    mockFetchGitLabMergeRequest(params),
...
jest.mock("@/lib/integrations/platforms/gitlab/instance-url", () => {
  const actual = jest.requireActual(
    "@/lib/integrations/platforms/gitlab/instance-url",
  );
  return {
    ...actual,
...
const integrationRow = {
  ...
  repositories: [
    { id: 7, name: "repo", full_name: PROJECT_PATH, private: true },
  ],
...
describe("addComment with an anchor (diff discussion)", () => {
  ...
  it("LEFT anchor positions on the old side with old_line", async () => {
    ...
    const position = (
      JSON.parse(String(init.body)) as { position: Record<string, unknown> }
    ).position;
  ...
  it("a RIGHT startLine range adds old_line beside new_line", async () => {
    await addComment({
      ...TARGET,
      body: "This block",
    ...
    expect(position.new_line).toBe(20);
    expect(position.old_line).toBe(10);
  });
  ...
  it("classifies a provider 400 (line outside the diff) as bad_request", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse({ message: "400 Bad request" }, 400),
    );
...
describe("submitReview", () => {
  ...
  it("the capability list excludes request_changes", () => {
    expect(GITLAB_MR_REVIEW_CAPABILITIES.reviewEvents).toEqual([
      "approve",
      "comment",
    ]);
  ...
  it("a mid-batch provider rejection surfaces the classified error and stops the batch", async () => {
    ...
    expect(
      fetchMock.mock.calls.some((entry) =>
        new URL(String(entry[0])).pathname.endsWith("/approve"),
      ),
    ).toBe(false);
    expect(mockCreateMRNote).not.toHaveBeenCalled();
  });
...
describe("resolveThread / unresolveThread", () => {
  ...
  it("refuses to resolve a non-resolvable discussion", async () => {
    ...
    await expect(
      resolveThread({ ...TARGET, discussionId: "disc-9" }),
...
describe("mergePullRequest", () => {
  ...
  it("refuses a stale revision with the exact reason and never merges", async () => {
    ...
    const mergeCall = fetchMock.mock.calls.find((call) =>
      String(call[0]).endsWith("/merge"),
    );
  ...
  it("surfaces a provider 409 as the same stale-head reason", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse({ message: "Branch cannot be merged" }, 409),
    );
...
describe("deleteBranch", () => {
  ...
  it("treats an already-deleted branch as a replay", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse({ message: "404 Branch Not Found" }, 404),
    );
...
describe("mutation failures reach the four mobile states", () => {
  it("classifies a 403 approve as non-retryable forbidden and leaks nothing", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(
        { message: "403 Forbidden — token glpat-secret denied" },
        403,
      ),
    );

    const error = await captureRejection(
      submitReview({ ...TARGET, event: "approve" }),
    );
