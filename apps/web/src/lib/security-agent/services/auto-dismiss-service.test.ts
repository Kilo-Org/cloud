...
>;
const mockResolveGitHubIntegrationForRepository = jest.fn() as jest.MockedFunction<
  typeof platformIntegrationsModule.resolveGitHubIntegrationForRepository
>;
...
jest.mock('@/lib/integrations/db/platform-integrations', () => ({
  resolveGitHubIntegrationForRepository: mockResolveGitHubIntegrationForRepository,
}));
...
function makeIntegration(installationId: string) {
  ...
  } as Awaited<ReturnType<typeof platformIntegrationsModule.resolveGitHubIntegrationForRepository>>;
}
...
describe('writebackDependabotDismissal', () => {
  it('dismisses a Dependabot alert on GitHub', async () => {
    ...
    mockResolveGitHubIntegrationForRepository.mockResolvedValue(makeIntegration('inst-123'));
    ...
    expect(mockResolveGitHubIntegrationForRepository).toHaveBeenCalledWith({
      owner: userOwner,
      repositoryFullName: 'acme/repo',
      expectedPlatformIntegrationId: 'integration-1',
    });
  ...
  it('uses unique repository fallback for legacy findings only', async () => {
    mockGetSecurityFindingById.mockResolvedValue(makeFinding({ platform_integration_id: null }));
    mockResolveGitHubIntegrationForRepository.mockResolvedValue(makeIntegration('legacy-inst'));

    await writebackDependabotDismissal('finding-1', userOwner, 'Legacy finding');

    expect(mockResolveGitHubIntegrationForRepository).toHaveBeenCalledWith(
      expect.objectContaining({ expectedPlatformIntegrationId: undefined })
    );
  });

  it('fails closed when the pinned integration is unavailable', async () => {
    mockGetSecurityFindingById.mockResolvedValue(makeFinding());
    mockResolveGitHubIntegrationForRepository.mockResolvedValue({
      success: false,
