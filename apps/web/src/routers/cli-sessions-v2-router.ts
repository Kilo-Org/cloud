...
import { getPinnedCliSessionGitHubIntegration } from '@/lib/integrations/platforms/github/cli-session-integration';
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (session.cloud_agent_session_id) {
        ...
        try {
          ...
          if (!pinnedIntegrationId) {
            throw new TRPCError({
              code: 'INTERNAL_SERVER_ERROR',
              message:
                'Could not resolve the GitHub installation pinned to this Cloud Agent session',
            });
          }
        }
        if (
      ...
      if (!integration?.platform_installation_id) {
        throw new TRPCError({
          code: 'BAD_REQUEST',
          message: 'Could not resolve the GitHub installation pinned to this session',
        });
      }
