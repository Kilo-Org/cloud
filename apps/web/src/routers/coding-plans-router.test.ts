...
describe('coding plans router', () => {
  ...
  it('authorizes managed usage through the original inventory assignment after BYOK changes', async () => {
    ...
    const otherUser = await insertTestUser();
    await uploadKeysToInventory(PLAN_ID, [inventoryEntry(managedKey)], {
      validateCredential: async () => true,
    });
    const ownerCaller = await createCallerForUser(owner.id);
    const otherCaller = await createCallerForUser(otherUser.id);
  ...
  it('keeps past-due and pending-cancellation subscriptions eligible but rejects canceled plans', async () => {
    ...
    await uploadKeysToInventory(PLAN_ID, [inventoryEntry(`sk-cp-state-${crypto.randomUUID()}`)], {
      validateCredential: async () => true,
    });
  ...
  it('fails safely for a corrupt inventory assignment without affecting database-only reads', async () => {
    ...
    await uploadKeysToInventory(PLAN_ID, [inventoryEntry(`sk-cp-corrupt-${crypto.randomUUID()}`)], {
      validateCredential: async () => true,
    });
    const caller = await createCallerForUser(owner.id);
    const activation = await caller.codingPlans.subscribe({
