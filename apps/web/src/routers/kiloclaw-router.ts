...
import { getKiloClawAgentCardOAuthConnection } from '@/lib/kiloclaw/agentcard-oauth-connections';
...
/** Maps a stored AgentCard OAuth connection (or none) to dashboard status fields. */
function agentCardStatusFields(
  connection: Awaited<ReturnType<typeof getKiloClawAgentCardOAuthConnection>>
): Pick<
  KiloClawDashboardStatus,
  'agentcardOAuthConnected' | 'agentcardOAuthStatus' | 'agentcardOAuthAccountEmail'
> {
  return {
    agentcardOAuthConnected: connection?.status === 'active',
    agentcardOAuthStatus: connection?.status ?? 'disconnected',
    agentcardOAuthAccountEmail: connection?.account_email ?? null,
  };
}
...
export const kiloclawRouter = createTRPCRouter({
  ...
  getStatus: baseProcedure.query(async ({ ctx }) => {
    ...
    // AgentCard connection status is sourced from Kilo's own DB, so it applies
    // to every instance — including local fake-seed instances, which return
    // early below and would otherwise never reflect a connection.
    const agentcardFields = agentCardStatusFields(
      await getKiloClawAgentCardOAuthConnection(instance.id)
    );

    if (isFakeSeedInstance(instance)) {
      return { ...createFakeSeedInstanceStatus(instance, legacyWorkerUrl), ...agentcardFields };
    }
