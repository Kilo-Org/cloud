...
const mockListGitLabBranches = jest.fn();
jest.mock('@/lib/integrations/gitlab-service', () => ({
  listGitLabBranches: (...a: unknown[]) => mockListGitLabBranches(...a),
}));
...
beforeEach(() => {
  ...
  mockListGitLabBranches.mockResolvedValue({
    branches: [
...
describe('organizationCloudAgentNextRouter.listRepositoryBranches', () => {
  ...
  it('lists GitLab branches with the acting user as the credential actor', async () => {
    ...
    expect(mockListGitLabBranches).toHaveBeenCalledWith(
      { type: 'org', id: ORG_ID },
      'int-1',
