...
describe('Vercel runtime upgrade', () => {
  async function readyCredential() {
    const caller = createCaller({ user: owner });
    mockSelectionValidation();
    await caller.add(setupInput);
    const current = await getCredential();
    if (!current) throw new Error('Expected credential');
    ...
    await db
      ...
      .set({
        ...
        setup_step: null,
        setup_error: null,
        runtime_snapshot_id: 'snapshot-current',
      ...
      .where(eq(organization_vercel_compute_credentials.id, current.id));
    return current;
  }

  it('starts an upgrade without removing the Ready snapshot', async () => {
    const caller = createCaller({ user: owner });
    const current = await readyCredential();
    mockStartBuild.mockClear();
    mockCleanupBuild.mockClear();

    const upgraded = await caller.upgradeRuntime({ organizationId });
    const row = await getCredential();

    expect(upgraded.setupStatus).toBe('ready');
    expect(upgraded.runtimeSnapshotId).toBe('snapshot-current');
    expect(upgraded.upgradeStatus).toBe('pending');
    expect(upgraded.buildGeneration).not.toBe(current.build_generation);
  ...
  it('marks upgrade failed without taking the org offline when start fails', async () => {
    const caller = createCaller({ user: owner });
    await readyCredential();
    mockStartBuild.mockRejectedValueOnce(new Error('build start failed'));

    await expect(caller.upgradeRuntime({ organizationId })).rejects.toMatchObject({
      message: 'Cloud Agent snapshot build could not be started',
    });
    expect(await getCredential()).toMatchObject({
      setup_status: 'ready',
      runtime_snapshot_id: 'snapshot-current',
      upgrade_status: 'failed',
      upgrade_error: 'setup_start_failed',
    });
  });

  it('reports an upgrade when the Worker digest differs', async () => {
    const caller = createCaller({ user: owner });
    await readyCredential();
    mockRuntimeIdentity.mockResolvedValueOnce({
      wrapperVersion: '2.4.0',
      releasedAt: '2026-09-08',
      digest: 'digest-new',
    });

    await expect(caller.getStatus({ organizationId })).resolves.toMatchObject({
      runtimeWrapperVersion: '2.4.0',
      runtimeReleasedAt: '2026-09-01',
      latestWrapperVersion: '2.4.0',
      latestReleasedAt: '2026-09-08',
