...
import {
  providerLedgerResourceKey,
  providerReviewRouter,
} from "./provider-review-router";
...
jest.mock("@kilocode/db/operation-ledger", () => ({
  ...
  markReconcilePending: (...args: unknown[]) =>
    mockMarkReconcilePending(...args),
  recordOperationAcceptance: (...args: unknown[]) =>
    mockRecordOperationAcceptance(...args),
}));
...
jest.mock("./organizations/utils", () => ({
  ensureOrganizationAccess: (...args: unknown[]) =>
    mockEnsureOrganizationAccess(...args),
}));
...
jest.mock("@/lib/provider-review/bitbucket-read", () => ({
  ...
  getMergeRestrictions: (...a: unknown[]) =>
    bitbucketRead.getMergeRestrictions(...a),
...
function admittedRow(
  overrides: Partial<OperationLedgerRow> = {},
): OperationLedgerRow {
  return {
    id: "row-1",
 ...
 */
function admittingOnce(
  admission: string,
  rowOverrides: Partial<OperationLedgerRow> = {},
): void {
  mockAdmitOperation.mockImplementationOnce(
    async (_db: unknown, args: any) => ({
      admission,
      row: admittedRow({
        intent: args.intent,
...
beforeEach(() => {
  ...
  mockMarkReconcilePending.mockResolvedValue(
    admittedRow({ status: "reconcile_pending" }),
  );
...
describe("providerReviewRouter inputs", () => {
  it("rejects host, token, instanceUrl, and userId fields on the GitLab arm", async () => {
    for (const smuggled of [
      ...
      await expect(
        caller.getPullRequest({ ...gitlabBase, ...smuggled }),
...
describe("providerReviewRouter ledger", () => {
  it("admits provider writes into the shared pr domain with a provider-tagged resource key", async () => {
    ...
    expect(
      expectedKey.startsWith(
        JSON.stringify(["gitlab", "", "group/sub/repo", 7]),
      ),
    ).toBe(true);
  });
  ...
  it("settles a completed write with the pr_operation_settled outbox event", async () => {
    ...
    const event = (mockSettleOperation.mock.calls[0][1] as { outboxEvent: any })
      .outboxEvent;
  ...
  it("marks the row reconcile-pending on a retryable provider failure and surfaces the ambiguous marker", async () => {
    gitlabWrite.addComment.mockRejectedValueOnce(
      new GitLabReviewError(
        "retryable",
        "Could not reach GitLab. Please try again.",
      ),
    );
    await expect(
      caller.addComment({
...
describe("providerReviewRouter inline anchors", () => {
  ...
  it("without an anchor the write payload and the fingerprint bytes stay unchanged", async () => {
    ...
    expect(gitlabWrite.addComment.mock.calls[0][0]).not.toHaveProperty(
      "anchor",
    );
  ...
  it("submitReview folds the comment batch into the write and the fingerprint", async () => {
    ...
    expect(gitlabWrite.submitReview).toHaveBeenCalledWith(
  ...
  it("a submit without comments keeps the legacy fingerprint bytes", async () => {
    ...
    expect(gitlabWrite.submitReview.mock.calls[0][0]).not.toHaveProperty(
      "comments",
    );
    const legacyKey = providerLedgerResourceKey(
      "submit_review",
...
describe("providerReviewRouter merge head fence", () => {
  ...
  it("reconciles a pending merge by re-reading through the owner-bound reader", async () => {
    ...
    gitlabRead.getMergeRequest.mockResolvedValueOnce(
      summaryFixture({ state: "merged" }),
    );
    await expect(
      caller.mergePullRequest({
        ...gitlabBase,
  ...
  it("a reconcile read showing a moved head settles failed confirmed_absent and refuses the merge", async () => {
    ...
    gitlabRead.getMergeRequest.mockResolvedValueOnce(
      summaryFixture({ headSha: "b".repeat(40) }),
    );
    await expect(
      caller.mergePullRequest({
        ...gitlabBase,
  ...
  it("a failed authoritative read stays reconcile-pending instead of settling absent", async () => {
    ...
    gitlabRead.getMergeRequest.mockRejectedValueOnce(
      new GitLabReviewError("not_found", "gone"),
    );
    await expect(
      caller.mergePullRequest({
        ...gitlabBase,
...
describe("providerReviewRouter capabilities", () => {
  it("answers GitLab with the MR capability list (no request-changes event)", async () => {
    await expect(
      caller.getCapabilities({ platform: "gitlab" }),
    ).resolves.toEqual(GITLAB_MR_REVIEW_CAPABILITIES);
    expect(GITLAB_MR_REVIEW_CAPABILITIES.reviewEvents).not.toContain(
      "request_changes",
    );
  });
  ...
  it("returns the capability reason for Bitbucket auto-merge without a ledger row", async () => {
    ...
    await expect(
      caller.disableAutoMerge({ ...bitbucketBase }),
