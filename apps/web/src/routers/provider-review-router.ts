...
const providerInlineAnchorInput = z
  ...
  .strict()
  .refine(
    (value) => value.startLine === undefined || value.startLine <= value.line,
    startLineOrderIssue,
  );
...
const providerIdentityInput = z.discriminatedUnion("platform", [
  z
    ...
    .strict(),
  z
    .object({ platform: z.literal("bitbucket"), organizationId: z.uuid() })
    .strict(),
]);

const GetPullRequestInput = providerRefInput({});

const ListFilesInput = providerRefInput({
  cursor: pageCursor,
...
function toProviderTrpcError(error: unknown): TRPCError {
  ...
  if (
    error instanceof GitLabReviewError ||
...
const PROVIDER_REPLAY_FAILED_MESSAGE =
 ...
 */
async function bestEffortLedgerWrite(
...
function providerSettledOutboxEvent(params: {
  ...
  return {
    ...
    properties: {
      ...
      ...(params.reconcileResult !== undefined
        ? { reconcile_result: params.reconcileResult }
        : {}),
...
async function failProviderRowAmbiguous(
  ...
  try {
    ...
    if (!updated || updated.status !== "reconcile_pending") {
      throw new Error(
        "markReconcilePending did not leave the row reconcile_pending",
      );
    }
  } catch (error) {
    console.error(
...
async function executeProviderWrite<T extends Record<string, unknown>>(
  ...
  try {
    ...
    await settleFailedProviderRow(
      base,
      row,
      outcomeCodeFromFailure(error, trpcError),
    );
    throw trpcError;
  }
...
function replaySettledProviderRow<T>(
  row: OperationLedgerRow,
): ReplayedResult<T> {
  ...
  throw new TRPCError({
    code: "BAD_REQUEST",
    message: PROVIDER_REPLAY_FAILED_MESSAGE,
  });
}
...
async function runProviderLedgerMutation<T>(
  ...
  if (
    admission.row.intent !== args.intent ||
    admission.row.resource_key !== args.resourceKey
  ) {
    throw new TRPCError({
      code: "CONFLICT",
...
async function runProviderMutation<T extends Record<string, unknown>>(args: {
  ...
  const resourceKey = providerLedgerResourceKey(
    args.intent,
    args.ref,
    args.fingerprintInput,
  );
  const execute = (row: OperationLedgerRow) =>
    executeProviderWrite(base, row, args.write);
  return runProviderLedgerMutation<T>({
    ...base,
    operationKey: args.operationKey,
    resourceKey,
    execute,
    reconcile: args.reconcileAmbiguous
      ? (row) => failProviderRowAmbiguous(base, row)
      : execute,
  });
}
...
async function reconcileMergeProviderRow<T extends Record<string, unknown>>(
  ...
  switch (state.kind) {
    case "merged": {
      ...
      await settleCompletedProviderRow(
        base,
        row,
        canonical,
        "confirmed_completed",
...
export const providerReviewRouter = createTRPCRouter({
   ...
   */
  listInbox: baseProcedure
    .input(ListInboxInput)
    .query(async ({ ctx, input }) => {
      if (input.platform === "gitlab") {
        const owner = await gitlabOwner(ctx, input);
        return providerCall(() =>
          gitlabRead.listInbox(owner, input.cursor, input.instanceHint),
        );
      }
      const owner = await bitbucketOwner(ctx, input);
  ...
  getMergeState: baseProcedure
    .input(GetMergeStateInput)
    .query(async ({ ctx, input }) => {
      if (input.platform === "gitlab") {
        const owner = await gitlabOwner(ctx, input);
        return providerCall(() =>
          gitlabRead.getMergeState(
            owner,
            input.projectPath,
            input.mrIid,
            input.instanceHint,
      ...
      const owner = await bitbucketOwner(ctx, input);
      return providerCall(() =>
        bitbucketRead.getMergeRestrictions(
          owner,
          input.workspace,
          input.repoSlug,
          input.prId,
  ...
  /** Post a comment. With an `anchor` it becomes a real inline discussion. */
   ...
   * An optional `comments` batch lands as real inline discussions before the
   * review state/summary note.
   */
  ...
  mergePullRequest: baseProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const mergeFields = {
        ...
        commitTitle:
          input.platform === "gitlab" ? input.commitTitle : undefined,
      ...
      if (input.platform === "gitlab") {
        ...
        const execute = (row: OperationLedgerRow) =>
          executeProviderWrite(base, row, write);
        return runProviderLedgerMutation({
          ...base,
          operationKey: input.operationKey,
          resourceKey: providerLedgerResourceKey(
            "merge",
            ref,
            fingerprintInput,
          ),
