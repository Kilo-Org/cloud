...
describe('mcp-catalog catalog', () => {
  ...
  describe('buildCatalogRows', () => {
    it('keeps only queries and drops denylisted top-level segments', () => {
      ...
      // `debug` is published (the allowlist leaf set only satisfies the rot
      // guard), so it waits for the LLM pass like any other summary-less leaf.
    ...
    it('keeps every query and only allowlisted mutations, dropping the denylist and subscriptions', () => {
      ...
      // allowlisted mutations without a summary wait for the LLM pass. A
      // published `debug` query without a summary waits alongside them.
      expect(missing.map(leaf => leaf.path)).toEqual([
        'debug.ping',
    ...
    it('keeps a debug leaf and marks it debug without an admin mark', () => {
      ...
      const row = rows[0];
      // The unsummarized allowlist leaves wait for the LLM pass; the debug row
      // itself is published.
