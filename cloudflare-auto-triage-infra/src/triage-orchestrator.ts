...
export class TriageOrchestrator extends DurableObject<Env> {
  ...
  private async answerQuestion(classification: ClassificationResult): Promise<void> {
    ...
    try {
      ...
      console.error('[TriageOrchestrator] Failed to post answer comment:', {
        ticketId: this.state.ticketId,
        error: error instanceof Error ? error.message : String(error),
      });
      await this.updateStatus('failed', {
        errorMessage: `Failed to post answer comment: ${error instanceof Error ? error.message : String(error)}`,
      });
      return;
    }
  ...
  private async requestClarification(classification: ClassificationResult): Promise<void> {
    ...
    const commentBody = [
      'We need a bit more information to triage this issue effectively. Could you please provide additional details?',
      '',
      classification.intentSummary,
      '',
      '*This request was generated by Kilo Auto-Triage.*',
    ].join('\n');

    try {
      await this.postComment(commentBody);
    } catch (error) {
      console.error('[TriageOrchestrator] Failed to post clarification comment:', {
        ticketId: this.state.ticketId,
        error: error instanceof Error ? error.message : String(error),
      });
      await this.updateStatus('failed', {
        errorMessage: `Failed to post clarification comment: ${error instanceof Error ? error.message : String(error)}`,
      });
      return;
    }
