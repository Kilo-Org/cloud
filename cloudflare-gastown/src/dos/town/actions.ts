      ...
      return async () => {
        try {
          ...
          if (status !== null) {
            ...
            ctx.insertEvent('poll_pr_nonnull', {
            ...
            if (refineryConfig.auto_resolve_pr_feedback) {
              ...
              if (
                ...
                // Emit feedback detected event — the synchronous handler will
                // check for existing feedback beads and create one if needed.
                ...
                ctx.insertEvent('poll_pr_feedback_checked', {
                  bead_id: action.bead_id,
                });
              }
            }
            ...
            if (
              ...
              if (allGreen) {
                // Emit timer set event — the synchronous handler will check
                // auto_merge_ready_since and either start the timer or emit
                // pr_auto_merge if the delay has elapsed.
                ctx.insertEvent('auto_merge_timer_set', {
                  bead_id: action.bead_id,
                  payload: {
                    pr_url: action.pr_url,
                ...
                ctx.insertEvent('auto_merge_timer_reset', {
                  bead_id: action.bead_id,
                });
              }
            }
            ...
            // Emit event so the synchronous handler increments the null counter
            // and fails the bead after PR_POLL_NULL_THRESHOLD consecutive nulls.
