...
api_post "/api/rigs/${RIG_ID}/agents" '{"role":"polecat","name":"Sender","identity":"sender-1"}'
SENDER_ID=$(echo "$HTTP_BODY" | jq -r '.data.id')

api_post "/api/rigs/${RIG_ID}/agents" '{"role":"polecat","name":"Receiver","identity":"receiver-1"}'
RECEIVER_ID=$(echo "$HTTP_BODY" | jq -r '.data.id')

echo "  Sending mail..."
api_post "/api/rigs/${RIG_ID}/mail" "$(jq -n --arg from "$SENDER_ID" --arg to "$RECEIVER_ID" \
  '{from_agent_id: $from, to_agent_id: $to, subject: "test", body: "hello"}')"
assert_status "201" "send mail"

echo "  Checking mail for receiver..."
api_get "/api/rigs/${RIG_ID}/agents/${RECEIVER_ID}/mail"
