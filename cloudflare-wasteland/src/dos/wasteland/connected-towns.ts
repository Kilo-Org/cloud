/**
 * Connected towns operations for WastelandDO.
 */
...
export function connectTown(
  ...
  return { town_id: townId, wasteland_id: wastelandId, connected_by: userId, connected_at: now };
}
...
export function listConnectedTowns(sql: SqlStorage): ConnectedTownRecord[] {
  const rows = [
    ...query(
      sql,
      /* sql */ `
  ...
  return ConnectedTownRecord.array().parse(rows);
}
