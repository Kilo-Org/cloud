...
async function cleanup() {
  ...
  if (orgIds.length > 0) {
    await db
      ...
      .where(inArray(kiloclaw_instances.organization_id, orgIds));
    await db
...
export async function run(): Promise<SeedResult> {
  ...
  const acme = await createOrg('Acme Corp (Teams active)', 'teams', { seatCount: 10 });
  ...
  const buildco = await createOrg('BuildCo (Teams active yearly)', 'teams', { seatCount: 25 });
  await addMember(buildco, david, 'owner');
  await addMember(buildco, eve);
  await addSubscription(buildco, 'active', { amountUsd: 7_500, seatCount: 25, billingCycle: 'yearly', createdDaysAgo: 180 });

  const codelab = await createOrg('CodeLab (Teams active small)', 'teams', { seatCount: 3 });
  ...
  const debtco = await createOrg('DebtCo (Teams past_due)', 'teams', { seatCount: 8 });
  ...
  await addSubscription(exstartup, 'active',   { amountUsd: 1485, seatCount: 15, createdDaysAgo: 240 });
  await addSubscription(exstartup, 'canceled', { amountUsd: 1485, seatCount: 15, createdDaysAgo: 45 });
  ...
  await addSubscription(megacorp, 'active', { amountUsd: 25_000, seatCount: 200, billingCycle: 'yearly', createdDaysAgo: 365 });
  ...
  await addSubscription(globaltech, 'active', { amountUsd: 8_000, seatCount: 50, createdDaysAgo: 120 });
  ...
  await addSubscription(oldenterprise, 'active', { amountUsd: 5_000, seatCount: 40, createdDaysAgo: 400 });
  await addSubscription(oldenterprise, 'ended',  { amountUsd: 5_000, seatCount: 40, createdDaysAgo: 60 });
