...
At most one non-ended agreement may exist for a parent organization. Commercial states are `pending_payment`, `active`, `cancel_at_period_end`, and `ended`. Manual, blocked, overallocated, and failed are processing conditions rather than commercial states. Provider subscription and add-on identities are unique among non-ended agreements so a parent can purchase again on the same seat subscription after an earlier agreement ends.
...
A child sub-org cannot detach, reparent, archive, or delete while its current effective plan, a future-effective plan, or a plan needed by an unresolved processing run assigns it nonzero Kilo Pass allocation. A parent organization owner or billing manager must first set the applicable allocations to zero. Superseded historical allocations and already granted pooled credits do not block the hierarchy change; shared balance cannot safely identify source-specific remainder for clawback.
...
Canonical qualifying debit ledger remains queryable by allocation container, timestamp, and stable transaction ID. It records request-level debits only for organizations that are current or planned allocation containers of a non-ended agreement; ordinary organizations continue using the canonical product-usage tables without duplicating every request into the Credit activity ledger. Member and admin Credit activity lists hide qualifying debit rows. Blocked run records original window immediately; repair reconstructs pooled spend from immutable debit records.
