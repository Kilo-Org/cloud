...
This audit covers native/mobile, Chat, Wasteland, and benchmark in the retained PR, their token-bearing requests, and the receiving authentication policies. It combines source tracing with automated route, storage and signed-token tests. It does not certify a physical-device or deployed provider/container smoke run. Cloud Agent transport lifecycle fixes, Gastown, Webhook Agent Ingest, Security Auto Analysis, and explicit resource delegation are owned separately.
...
- Chat/Wasteland/benchmark: exercise the actual receiving deployments and installed client versions before enabling each family. Source-level coverage and mocked tests do not prove the deployed aliases, images or third-party services are aligned.
