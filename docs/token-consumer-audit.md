...
This audit covers native/mobile, Chat, Wasteland, explicit delegation, benchmark, and Security Auto Analysis in the retained PR, their token-bearing requests, and the receiving authentication policies. It combines source tracing with automated route, storage and signed-token tests. It does not certify a physical-device or deployed provider/container smoke run. Cloud Agent transport lifecycle fixes, Gastown, and Webhook Agent Ingest are owned separately.
...
| Automation callbacks and external services | Security analysis/remediation completion, notifications, GitHub operations | Attempt/session-bound callback HMAC, internal secret or GitHub credential as appropriate. None is replaced with a Kilo control JWT. |
...
Automation checks use real local token signing, verification and runtime admission with mocked external boundaries. The Security Auto Analysis Worker gate is exercised independently of Web settings. Checks cover null peppers, legacy credentials, audience separation, bounded lifetime, renewal and revocation. No global key/pepper mutation or broad legacy fallback is introduced.
