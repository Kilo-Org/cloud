...
describe('updateToolsMdGitHubSection', () => {
  it('adds GitHub section when GITHUB_TOKEN is set', () => {
    const harness = fakeDeps();
    ...
    expect(harness.writeCalls[0]!.data).toContain('<!-- BEGIN:github -->');
    expect(harness.writeCalls[0]!.data).toContain('gh');
    expect(harness.writeCalls[0]!.data).toContain('<!-- END:github -->');
  });
  ...
  it('removes stale section when token is absent', () => {
    ...
    expect(harness.writeCalls).toHaveLength(1);
    expect(harness.writeCalls[0]!.data).not.toContain('<!-- BEGIN:github -->');
  });
