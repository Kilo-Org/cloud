...
export class KiloClawInstance extends DurableObject<KiloClawEnv> {
  ...
  async getDebugState(): Promise<{
    ...
    console.log('[vol-usage] DO.getDebugState disk stats:', JSON.stringify({ diskUsedBytes: this.s.diskUsedBytes, diskTotalBytes: this.s.diskTotalBytes, sandboxId: this.s.sandboxId }));
  ...
  async recordDiskStats(usedBytes: number, totalBytes: number): Promise<void> {
    await this.loadState();
    console.log('[vol-usage] DO.recordDiskStats called:', JSON.stringify({ usedBytes, totalBytes, sandboxId: this.s.sandboxId }));
