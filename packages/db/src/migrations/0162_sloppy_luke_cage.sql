...
CREATE UNIQUE INDEX "UQ_security_remediation_attempts_finding_fingerprint_terminal" ON "security_remediation_attempts" USING btree ("finding_id","analysis_fingerprint") WHERE "security_remediation_attempts"."status" IN ('queued', 'launching', 'running', 'pr_opened', 'blocked', 'no_changes_needed');--> statement-breakpoint
