...
ALTER TABLE "security_findings" DROP CONSTRAINT "uq_security_findings_source";--> statement-breakpoint
...
CREATE UNIQUE INDEX "uq_security_findings_user_source" ON "security_findings" USING btree ("owned_by_user_id","repo_full_name","source","source_id") WHERE "security_findings"."owned_by_user_id" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX "uq_security_findings_org_source" ON "security_findings" USING btree ("owned_by_organization_id","repo_full_name","source","source_id") WHERE "security_findings"."owned_by_organization_id" IS NOT NULL;