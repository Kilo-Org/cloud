...
CREATE UNIQUE INDEX CONCURRENTLY "UQ_cloud_agent_code_reviews_manual_user_repo_pr_sha" ON "cloud_agent_code_reviews" USING btree ("owned_by_user_id","repo_full_name","pr_number","head_sha") WHERE "cloud_agent_code_reviews"."manual_config" IS NOT NULL AND "cloud_agent_code_reviews"."owned_by_user_id" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX CONCURRENTLY "UQ_cloud_agent_code_reviews_manual_org_repo_pr_sha" ON "cloud_agent_code_reviews" USING btree ("owned_by_organization_id","repo_full_name","pr_number","head_sha") WHERE "cloud_agent_code_reviews"."manual_config" IS NOT NULL AND "cloud_agent_code_reviews"."owned_by_organization_id" IS NOT NULL;--> statement-breakpoint
