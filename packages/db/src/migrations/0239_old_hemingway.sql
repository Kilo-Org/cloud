...
ALTER TABLE "github_connection_attempts" ADD CONSTRAINT "github_connection_attempts_completed_integration_id_platform_integrations_id_fk" FOREIGN KEY ("completed_integration_id") REFERENCES "public"."platform_integrations"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "IDX_github_connection_attempts_expires_at" ON "github_connection_attempts" USING btree ("expires_at");--> statement-breakpoint
CREATE INDEX "IDX_github_connection_attempts_completed_integration_id" ON "github_connection_attempts" USING btree ("completed_integration_id");
-->  statement-breakpoint
WITH eligible AS (
	SELECT pi.*, COALESCE(pi.github_app_type, 'standard') AS effective_app_type,
		COUNT(*) OVER (PARTITION BY COALESCE(pi.github_app_type, 'standard'), pi.platform_installation_id) AS peer_count
	...
	WHERE pi.platform = 'github' AND pi.integration_type = 'app'
		...
		AND pi.github_disconnected_at IS NULL AND pi.suspended_at IS NULL AND pi.auth_invalid_at IS NULL
		AND NOT (COALESCE(pi.metadata, '{}'::jsonb) ? 'github_dedup')
)
INSERT INTO github_app_installations (
	github_app_type, installation_id, account_id, account_login, permissions, scopes,
	repository_access, repositories, repositories_synced_at, lifecycle_state,
	observed_at, created_at, updated_at
)
SELECT effective_app_type, platform_installation_id, platform_account_id,
	platform_account_login, permissions, scopes, repository_access, repositories,
	repositories_synced_at, 'active', COALESCE(updated_at, now()), now(), now()
FROM eligible WHERE peer_count = 1
ON CONFLICT (github_app_type, installation_id) DO NOTHING;
-->  statement-breakpoint
WITH eligible AS (
	SELECT pi.id, COALESCE(pi.github_app_type, 'standard') AS effective_app_type,
...
SET github_installation_id = canonical.id, updated_at = now()
FROM eligible e
JOIN github_app_installations canonical
	ON canonical.github_app_type = e.effective_app_type
	AND canonical.installation_id = e.platform_installation_id
WHERE e.peer_count = 1 AND pi.id = e.id AND pi.github_installation_id IS NULL;
