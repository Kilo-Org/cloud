...
it('reads the thinking off a frame whose content is present but empty', () => {
  /* A provider puts `content: ""` on the frames that carry the thinking: the
     empty string is a placeholder, not a word. Reading it as the answer would
     shadow the thinking on the same frame. */
  expect(
    completionsWire.toDelta({ choices: [{ delta: { content: '', reasoning: 'hmm' } }] })
  ).toEqual({ kind: 'reasoning', text: 'hmm' });
  expect(
    completionsWire.toDelta({ choices: [{ delta: { content: '', reasoning_content: 'hmm' } }] })
  ).toEqual({ kind: 'reasoning', text: 'hmm' });

  /* A frame whose content is empty and that carries nothing else says nothing. */
  expect(completionsWire.toDelta({ choices: [{ delta: { content: '' } }] })).toBeUndefined();
});
