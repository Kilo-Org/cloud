...
for (const [file, paths, filterName] of [
  ...
  test(`${file} admits main and stack PRs, keeps main-only pushes and relevant paths`, () => {
    ...
    if (filterName) {
      ...
      // The mobile workflow admits every PR: the suite jobs are required
      // checks and gate their own work on the internal change-detection step.
      // Main pushes stay filtered to the suite's own paths.
      assert.equal(
