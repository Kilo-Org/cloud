 ...
 * manager, restore-session helper) can reconstruct it without threading the
 * value through every call. The file is created by `bringUpDevContainer` and
 * removed by `teardownDevContainer`.
 */
export function getDevContainerOverridePath(agentSessionId: string): string {
 ...
 */
export const KILO_CLI_VERSION = '7.1.23';
...
export async function bringUpDevContainer(
  ...
  // Since our generated file lives outside the workspace, merge it ourselves so
  // the user's `image`/`dockerFile`/`dockerComposeFile` stays intact.
  const overridePath = getDevContainerOverridePath(agentSessionId);
  ...
  return {
    ...
    teardown: () => teardownDevContainer(session, workspacePath, agentSessionId, dockerEnv),
  };
}
 ...
 * Strictly additive for `mounts`/`runArgs`/`remoteEnv` (we never touch
...
export async function writeMergedOverrideConfig(
  ...
  const { outputPath, baseConfig, sessionHome, wrapperPort, agentSessionId } = opts;
...
export function mergeDevContainerConfig(
  baseConfig: DevContainerJson,
  opts: { sessionHome: string; wrapperPort: number; agentSessionId: string }
): DevContainerJson {
...
__KILO_READ_DEVCONTAINER_EOF__`;
  ...
  return JSON.parse(result.stdout ?? '{}') as DevContainerJson;
}
...
async function teardownDevContainer(
  ...
  const downCmd = `devcontainer down --workspace-folder ${shellQuote(workspacePath)}`;
  try {
  ...
  // Clean up the merged override config dir we kept alive for `devcontainer
  // exec --config`. Best-effort — `/tmp` gets reaped on sandbox sleepAfter.
  const overrideDir = getDevContainerOverridePath(agentSessionId).replace(/\/[^/]+$/, '');
  try {
    await session.exec(`rm -rf ${shellQuote(overrideDir)}`);
  } catch {
    // tmp gets reaped on sandbox sleepAfter anyway.
  }
}
