...
} from '../byoc/vercel-runtime-identity.js';
import {
...
const NAMED_BUILD_FAILURES = new Set([
  'snapshot_result_ambiguous',
  'snapshot_result_missing',
  'setup_command_failed',
  'validator_call_home_failed',
  'validator_call_home_ended',
  'validator_call_home_ambiguous',
...
function failureCode(error: unknown): string {
  ...
  if (error instanceof ByocCredentialResolverError) return 'byoc_credential_unusable';
  ...
  if (error instanceof Error && NAMED_BUILD_FAILURES.has(error.message)) return error.message;
...
function logSnapshotBuildFailure(
  state: BuildRow,
  error: unknown,
  ...
  logger
    .withFields({
      ...
      ...(error instanceof VercelSandboxRestError
        ? {
            vercelKind: error.kind,
            vercelOperation: error.operation,
            vercelStatus: error.status,
          }
        : {}),
      ...extra,
    })
    .error('BYOC Vercel snapshot build failed');
}
...
export class VercelSnapshotBuild extends DurableObject<Env> {
        ...
        if (command.exitCode !== null) {
          logger
            .withFields({
              ...
              buildGeneration: state.build_generation,
              step: state.step,
              diagnosticEvent: 'validator_command_exited',
              expectedExitCode: 'null',
              observedExitCode: command.exitCode,
            })
            .warn('BYOC Vercel snapshot build check failed');
        ...
        if (status.physical !== 'running') {
          logger
            .withFields({
              ...
              buildGeneration: state.build_generation,
              step: state.step,
              diagnosticEvent: 'validator_physical_not_running',
              expectedPhysical: 'running',
              observedPhysical: status.physical,
              observedConnection: status.connection,
            })
            .warn('BYOC Vercel snapshot build check failed');
          throw new Error('validator_call_home_failed');
        }
  ...
  private projection(
    ...
    return {
      ...
      runtimeWrapperVersion: ready ? (identity?.wrapperVersion ?? null) : null,
      runtimeReleasedAt: ready ? (identity?.releasedAt ?? null) : null,
      runtimeDigest: ready ? (identity?.digest ?? null) : null,
    };
  }
  ...
  private async handleFailure(owner: BuildRow, error: unknown): Promise<void> {
    ...
    if (terminal || (isRetryable(error) && attempt <= MAX_RETRIES)) {
      logSnapshotBuildFailure(state, error, 'retry');
      const retrying = {
        ...state,
    ...
    logSnapshotBuildFailure(state, error, 'terminal');
