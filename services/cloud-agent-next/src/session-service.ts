...
export class SessionService {
  ...
  async resolveWorkspaceTokens(
    ...
    let gitToken = overrides?.gitToken ?? git?.token;
    let gitlabTokenManaged = git?.type === 'gitlab' ? git.gitlabTokenManaged : undefined;
    if (
      git?.url &&
      repositoryPlatform(metadata) === 'gitlab' &&
      !overrides?.gitToken &&
      (git.type !== 'gitlab' || git.gitlabTokenManaged !== false)
    ) {
      if (!env.GIT_TOKEN_SERVICE) {
        throw ExecutionError.invalidRequest('Git token service is not configured');
      }
      ...
      if (result.success) {
        ...
        logger
          .withFields({ reason: result.reason, sessionId: metadata.identity.sessionId })
  ...
  async buildWrapperSessionReadyAndPromptRequests(
    ...
    promptRequest: WrapperPromptRequest;
    ...
    const { sandboxId, metadata, repositoryAuthOverrides } = workspace;
    ...
    const resolvedTokens = await this.resolveWorkspaceTokens(
      env,
      metadata,
      repositoryAuthOverrides
    );
    ...
    const repo = this.buildWrapperRepoSource(
      env,
      metadata,
      resolvedTokens,
      repositoryAuthOverrides
    ...
    const attachments = await buildSignedImagePromptAttachments({
      env,
      userId,
      sessionId,
      images: turn.images,
    ...
    return { readyRequest, promptRequest, ready, context };
  }
    ...
    if (git) {
      ...
      return {
        ...
        ...(repositoryShallow(metadata) !== undefined
          ...
          : {}),
        refreshRemote: Boolean(tokenOverrides?.gitToken || isManagedGitLab),
      };
    }
    ...
    if (github) {
      ...
      return {
        ...
        ...(repositoryShallow(metadata) !== undefined
          ...
          : {}),
        ...(gitAuthor ? { gitAuthor } : {}),
        refreshRemote: Boolean(tokenOverrides?.githubToken || github.githubInstallationId),
      };
    }
  ...
  async prepareWorkspace(options: PrepareWorkspaceOptions): Promise<PreparedWorkspace> {
    const { sandbox, sandboxId, userId, sessionId, env, metadata, tokenOverrides, onProgress } =
      options;
    ...
    const resolvedTokens = await this.resolveWorkspaceTokens(env, metadata, tokenOverrides);
    const github = githubRepository(metadata);
    const git = gitRepository(metadata);
    const platform = repositoryPlatform(metadata);
    ...
    if (await this.workspaceHasGit(sandbox, workspacePath)) {
      ...
      await this.refreshGitRemoteToken(session, context, metadata, resolvedTokens, tokenOverrides);
   ...
   * managed by us. Explicit caller-supplied overrides also trigger a rewrite.
   */
  private async refreshGitRemoteToken(
    ...
    const github = githubRepository(metadata);
    if (github) {
      const githubToken = tokenOverrides?.githubToken ?? tokens.githubToken;
      const shouldRefresh =
        githubToken !== undefined &&
        (tokenOverrides?.githubToken !== undefined || github.githubInstallationId !== undefined);
      if (shouldRefresh) {
    ...
    const git = gitRepository(metadata);
    if (git) {
      const gitToken = tokenOverrides?.gitToken ?? tokens.gitToken;
      const isManagedGitLab =
        repositoryPlatform(metadata) === 'gitlab' &&
        (git.type !== 'gitlab' || git.gitlabTokenManaged !== false);
      const shouldRefresh =
        gitToken !== undefined && (tokenOverrides?.gitToken !== undefined || isManagedGitLab);
      if (shouldRefresh) {
        await updateGitRemoteToken(
          session,
          context.workspacePath,
