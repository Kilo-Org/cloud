...
describe('queueMessage', () => {
  it('returns the DO result mapped to an ExecutionResponse on success', async () => {
    ...
    expect(request?.kind === 'user-message' ? request.message.id : undefined).toBeUndefined();
  });
  ...
  it('forwards the caller messageId when provided', async () => {
    ...
    await queueMessage(
      {
        ...
        message: { id: 'msg_018f1e2d3c4bAbCdEfGhIjKlMn', prompt: 'hello' },
      },
    ...
    const request = queueSessionMessage.mock.calls[0]?.[0] as
      | QueueSessionMessageRequest
      | undefined;
    expect(request?.kind === 'user-message' ? request.message.id : undefined).toBe(
    ...
    await queueMessage(
      {
        ...
        tokenOverrides: { githubToken: 'github-token', gitToken: 'git-token' },
      },
