 ...
 * path. Callers that already hold an explicit token may pass it through
 * `githubToken`/`gitToken` and it will be forwarded verbatim.
 *
...
import type { AcceptedPromptTurn, PromptSubmission } from '../execution/types.js';
...
export type SessionRegistrationResult = {
   ...
   */
  initialTurn: AcceptedPromptTurn;
};

function acceptInitialPrompt(initialPrompt: PromptSubmission): AcceptedPromptTurn {
  return {
    messageId: initialPrompt.id ?? createMessageId(),
    prompt: initialPrompt.prompt,
    images: initialPrompt.images,
  };
}

export function promptSubmissionFromAcceptedTurn(turn: AcceptedPromptTurn): PromptSubmission {
  return {
    id: turn.messageId,
    prompt: turn.prompt,
    images: turn.images,
...
export async function ensureSessionRegistered(
  ...
  const initialTurn = acceptInitialPrompt(input.initialPrompt);
