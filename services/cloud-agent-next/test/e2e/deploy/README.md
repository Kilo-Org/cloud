...
|---|---|---|
| `E2E_USER_ID` | no | Enrollment id written to both `CONTROL_PLANE_IDS` and `WORKTREE_CREATION_ENABLED_IDS`; defaults to `*` (every authenticated user is enrolled). It no longer authorizes the `/__e2e/*` surface — the e2e internal secret does — and the deployed test Worker writes to production Postgres and R2. |
