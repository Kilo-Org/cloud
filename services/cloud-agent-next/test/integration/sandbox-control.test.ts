...
} from '../../src/sandbox-containers/containers-billing.js';
import type {
...
import { MANAGED_SCM_OUTBOUND_HANDLER, deriveSandboxAllocationId } from '../../src/sandbox-id.js';
...
async function seedGrant(
  ...
  options: { containmentEnabled?: boolean } = {}
...
function containersRecordAck(): RecordAck {
  return { intervalId: 'interval-containers', durable: 'pg', dedup: false };
}

class ContainersIntegrationMeter implements ContainerUsageRpcMethods {
  heartbeatBudget: BudgetVerdict = { verdict: 'continue' };

  async recordStart(_input: RecordStartInput): Promise<RecordStartResult> {
    return { success: true, ack: containersRecordAck() };
  }

  async recordHeartbeat(_input: RecordHeartbeatInput): Promise<HeartbeatAck> {
    return { ...containersRecordAck(), budget: this.heartbeatBudget };
  }

  async recordStop(_input: RecordStopInput): Promise<RecordAck> {
    return containersRecordAck();
  }
}
...
describe('SandboxControl mandatory worktree credentials', () => {
  it('joins authoritative session metadata, native containment, wrapper handshake, and sanitized attach', async () => {
    ...
    try {
      ...
      expect(request.payload).not.toHaveProperty('captureNativeRuntimeId');
  ...
  it('requests native runtime id capture on attach when the wrapper advertises it', async () => {
    const fixture = await credentialFixture();
    const { control, registration, containers } = fixture;
    const input = credentialInput(registration);
    const ready = await control.ensureReady({ ...input, allowCreate: true });
    ...
    if (!payload?.kilo) throw new Error('Missing contained readiness attachment');
    const launch = containers.launches[0];
    if (!launch) throw new Error('Missing contained launch');
    const ws = await connect(launch.env.SANDBOX_CONTROL_CREDENTIAL, fixture.sandboxId);
    try {
      await completeHello(ws, 'hello-native-capture', {
        providerInstanceId: launch.env.PROVIDER_INSTANCE_ID,
        sessionOperationResults: true,
      ...
      const request = JSON.parse(await inbound) as {
        requestId: string;
        operation: string;
        payload: SessionAttachPayload & { captureNativeRuntimeId?: boolean };
      };
      expect(request).toMatchObject({ operation: 'session.attach', payload });
      expect(request.payload.captureNativeRuntimeId).toBe(true);
      ws.send(JSON.stringify({ type: 'response', requestId: request.requestId, ok: true }));
      await expect(pending).resolves.toMatchObject({ ok: true });
    } finally {
      ws.close();
