...
describe('PR poll error discrimination (#3149)', () => {
  ...
  async function setupMrBeadWithPrUrl(prUrl: string) {
    ...
    // Put the MR bead in_progress so the reconciler schedules a poll_pr
    // action on the next alarm tick (the PR URL was stored on the review
    // metadata entry by agentDone above).
