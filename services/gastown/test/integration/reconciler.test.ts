...
describe('Reconciler', () => {
  ...
  describe('reconcileReviewQueue Rule 6: refinery re-dispatch limits', () => {
    it('should fail MR bead after refinery exceeds max dispatch attempts', async () => {
      // configureRig stores the dispatch-time rig config (addRig only inserts
      // the SQL row). Without it, dispatchAgent returns before incrementing
      // the bead's dispatch_attempts.
      ...
      // First refinery start already incremented the MR bead's dispatch_attempts.
      // Cap the rig at 1 so the next idle re-dispatch hits the per-bead breaker
      // through production config rather than a test-only SQL stamp.
      ...
      // Run alarm — Rule 6 should see dispatch_attempts >= max and fail the MR bead
      await runDurableObjectAlarm(town);
