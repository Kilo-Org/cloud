...
describe('Reconciler', () => {
  ...
  describe('reconcileReviewQueue Rule 6: refinery re-dispatch limits', () => {
    it('should fail MR bead after refinery exceeds max dispatch attempts', async () => {
      // Stored rig config so the dispatch path runs and counts attempts.
      ...
      // The refinery dispatch above counted one attempt on the MR bead. Rule 6
      // compares the bead's own attempt count against the rig's cap, so lower
      // the rig cap to that count to put the bead at its limit.
      ...
      // Run alarm — Rule 6 should see the bead at its dispatch cap and fail it
      await runDurableObjectAlarm(town);
