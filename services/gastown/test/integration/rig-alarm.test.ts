...
describe('Town DO Alarm', () => {
  ...
  beforeEach(async () => {
    ...
    // The alarm loop and cadence helpers bail out without a stored town id.
  ...
  describe('schedule pending work', () => {
    it('should mark the agent working but not start a container without rig config', async () => {
      const agent = await town.registerAgent({
        role: 'polecat',
        name: 'P1',
      ...
      // Run alarm — no rig config, so the container never starts. The
      // reconciler still marks the agent working synchronously; it returns to
      // idle only through reconcileAgents once heartbeats are missing.
      await runDurableObjectAlarm(town);
    ...
    it('should attempt to dispatch idle agents with hooked beads', async () => {
      ...
      // will fail, but the agent is marked working synchronously and stays
      // working until reconcileAgents resets it.
      await runDurableObjectAlarm(town);
  ...
  describe('witness patrol via alarm', () => {
    it('should still detect dead agents when alarm fires', async () => {
      ...
      // Give the agent an activity timestamp so the GC does not treat a null
      // last_activity_at as long-dead and delete it during the alarm.
      await town.touchAgentHeartbeat(agent.id);
  ...
  describe('end-to-end alarm flow', () => {
    it('should handle the full bead → hook → alarm → patrol cycle', async () => {
      ...
      // agent is marked working and the container start fails)
