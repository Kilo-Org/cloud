...
describe('Town DO Alarm', () => {
  ...
  describe('schedule pending work', () => {
    it('should not dispatch agents without rig config', async () => {
      ...
      // Run alarm — no rig config, so container start is skipped, but the
      // dispatch_agent action still marks the agent working (I/O-gate).
      await runDurableObjectAlarm(town);
    ...
    it('should attempt to dispatch idle agents with hooked beads', async () => {
      ...
      // Run alarm — mock container accepts start, so the agent is dispatched
      await runDurableObjectAlarm(town);
  ...
  describe('end-to-end alarm flow', () => {
    it('should handle the full bead → hook → alarm → patrol cycle', async () => {
      ...
      // hookBead arms alarm — mock container accepts start, agent goes working
      ...
      // agentDone is event-only — drain the alarm to apply it
      await runDurableObjectAlarm(town);
