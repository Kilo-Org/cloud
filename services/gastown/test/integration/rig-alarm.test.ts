...
describe('Town DO Alarm', () => {
  ...
  beforeEach(async () => {
    ...
    // setTownId is required for armAlarmIfNeeded to arm an alarm: a DO with no
    // town:id is treated as destroyed and never (re-)arms.
  ...
  describe('schedule pending work', () => {
    it('should record a dispatch attempt but not start a container without rig config', async () => {
      const agent = await town.registerAgent({
        role: 'polecat',
        name: 'P1',
      ...
      // Run alarm — no rig config, so the container cannot be started
      await runDurableObjectAlarm(town);

      // The reconciler's dispatch action marks the agent 'working' before the
      // container start; dispatchAgent then refuses because there is no rig
      // config, so no container process ever starts.
    ...
    it('should attempt to dispatch idle agents with hooked beads', async () => {
      ...
      // will fail, but the dispatch attempt is still recorded
      await runDurableObjectAlarm(town);

      // Dispatch marks the agent 'working' before the (failing) container start
      const updatedAgent = await town.getAgentAsync(agent.id);
      expect(updatedAgent?.status).toBe('working');
    });
  });
  ...
  describe('end-to-end alarm flow', () => {
    it('should handle the full bead → hook → alarm → patrol cycle', async () => {
      ...
      // hookBead arms alarm — run it (container unavailable in tests, so the
      // dispatch attempt marks the agent 'working' but no container starts)
      ...
      // agentDone is event-only: the review-queue transition happens when
      // the alarm drains the agent_done event below.
      await town.agentDone(agent.id, {
        branch: 'feature/e2e',
      ...
      // Run alarm — drains agent_done and processes the review queue entry
      // (refinery dispatch will fail at container level but that's expected
      // in tests)
      await runDurableObjectAlarm(town);

      // The agent is unhooked by the drained event and returns to idle — the
      // polecat's work is done; only its container process still has to exit.
