...
describe('TownDO', () => {
  ...
  describe('hooks', () => {
    it('should hook and unhook a bead', async () => {
      ...
      // 'in_progress' only when dispatch starts the agent's container.
  ...
  describe('prime', () => {
    it('should assemble prime context for an agent', async () => {
      ...
      // prime() delivers pending mail (readAndDeliverMail), so nothing is left
  ...
  describe('bead events', () => {
    ...
    it('should write events on hookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // newest first: hooked, created (no status_changed — bead stays open)
    ...
    it('should write closed event on closeBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // closeBead is updateBeadStatus('closed') — newest first: status_changed, created
      expect(events).toHaveLength(2);
      expect(events[0].event_type).toBe('status_changed');
      expect(events[0].old_value).toBe('open');
