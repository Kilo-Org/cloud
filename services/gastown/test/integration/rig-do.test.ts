...
describe('TownDO', () => {
  ...
  describe('hooks', () => {
    it('should hook and unhook a bead', async () => {
      ...
      // hookBead assigns the agent but leaves the bead 'open'; it moves to
      // 'in_progress' only when the container actually starts.
  ...
  describe('bead status', () => {
    ...
    it('should filter beads by status', async () => {
      ...
      // Agents are themselves beads (type 'agent'), so scope the filter to
      // the task beads this test creates.
  ...
  describe('prime', () => {
    it('should assemble prime context for an agent', async () => {
      ...
      // prime reads and delivers mail — the mailbox is drained by the call
  ...
  describe('bead events', () => {
    ...
    it('should write events on hookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first. hookBead leaves the bead 'open', so it writes only
      // 'hooked' (no status_changed until the container starts).
      expect(events).toHaveLength(2);
      expect(events[0].event_type).toBe('hooked');
      expect(events[0].agent_id).toBe(agent.id);
      expect(events[0].new_value).toBe(agent.id);
      expect(events[1].event_type).toBe('created');
    });
    ...
    it('should write events on unhookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first: unhooked + hooked + created
      expect(events).toHaveLength(3);
      expect(events[0].event_type).toBe('unhooked');
    });
    ...
    it('should write events on updateBeadStatus', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first: status_changed + created
      expect(events).toHaveLength(2);
    ...
    it('should write closed event on closeBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first: closeBead delegates to updateBeadStatus, so the
      // transition is recorded as a 'status_changed' event.
      expect(events).toHaveLength(2);
      expect(events[0].event_type).toBe('status_changed');
      expect(events[0].new_value).toBe('closed');
    });
