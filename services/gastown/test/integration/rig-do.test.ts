...
describe('TownDO', () => {
  ...
  describe('hooks', () => {
    it('should hook and unhook a bead', async () => {
      ...
      // hookBead assigns the agent but leaves the bead open — the bead moves
      // to in_progress only when the agent's container actually starts.
  ...
  describe('prime', () => {
    it('should assemble prime context for an agent', async () => {
      ...
      // prime() reads and delivers undelivered mail, so the mailbox is empty
      // afterwards.
  ...
  describe('bead events', () => {
    ...
    it('should write events on hookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first: hooked + created. hookBead assigns the agent but leaves
      // the bead open, so there is no status_changed event.
    ...
    it('should write events on unhookBead', async () => {
      ...
      expect(events[2].event_type).toBe('created');
    });
    ...
    it('should write closed event on closeBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // Newest first: status_changed(open→closed) + created
      expect(events).toHaveLength(2);
      expect(events[0].event_type).toBe('status_changed');
