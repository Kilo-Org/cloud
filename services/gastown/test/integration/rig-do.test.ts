...
describe('TownDO', () => {
  ...
  describe('hooks', () => {
    it('should hook and unhook a bead', async () => {
      ...
      // Hook assigns the agent but leaves the bead 'open'; it transitions to
      // 'in_progress' only when the agent's container actually starts.
      expect(hookedBead?.status).toBe('open');
  ...
  describe('bead status', () => {
    ...
    it('should filter beads by status', async () => {
      ...
      const openBeads = await town.listBeads({ type: 'issue', status: 'open' });
      expect(openBeads).toHaveLength(1);
      expect(openBeads[0].title).toBe('Open bead');

      const closedBeads = await town.listBeads({ type: 'issue', status: 'closed' });
  ...
  describe('review queue', () => {
    ...
    it('should create escalation bead on merge conflict via completeReviewWithResult', async () => {
      ...
      expect(escalations[0].title).toContain('Merge conflict:');
      expect(escalations[0].title).toContain('CONFLICT (content)');
  ...
  describe('prime', () => {
    it('should assemble prime context for an agent', async () => {
      ...
      // Prime delivers undelivered mail (readAndDeliverMail closes the message
      // beads), so a second checkMail finds nothing pending.
  ...
  describe('bead events', () => {
    ...
    it('should write events on hookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // created + hooked (hookBead no longer changes the bead status; the
      // bead stays 'open' until the agent's container starts). Newest first.
      expect(events).toHaveLength(2);
      const hooked = events.find(e => e.event_type === 'hooked');
      expect(events.find(e => e.event_type === 'created')).toBeTruthy();
      expect(hooked).toBeTruthy();
      expect(hooked!.agent_id).toBe(agent.id);
      expect(hooked!.new_value).toBe(agent.id);
    });
    ...
    it('should write events on unhookBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // created + hooked + unhooked (newest first)
      expect(events).toHaveLength(3);
      expect(events.find(e => e.event_type === 'unhooked')).toBeTruthy();
    });
    ...
    it('should write events on updateBeadStatus', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // created + status_changed (newest first)
      expect(events).toHaveLength(2);
      const statusChanged = events.find(e => e.event_type === 'status_changed');
      expect(statusChanged).toBeTruthy();
      expect(statusChanged!.old_value).toBe('open');
      expect(statusChanged!.new_value).toBe('in_progress');
    });
    ...
    it('should write closed event on closeBead', async () => {
      ...
      const events = await town.listBeadEvents({ beadId: bead.bead_id });
      // created + status_changed(open→closed) — closeBead is an
      // updateBeadStatus call, so it logs a status_changed event.
      expect(events).toHaveLength(2);
      const closed = events.find(e => e.event_type === 'status_changed');
      expect(closed).toBeTruthy();
      expect(closed!.new_value).toBe('closed');
    });
