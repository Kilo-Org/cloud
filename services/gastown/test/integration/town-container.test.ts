...
describe('Heartbeat Endpoint', () => {
  ...
  it('should update agent activity via heartbeat', async () => {
    ...
    const createRes = await SELF.fetch(api(`/api/towns/${tid}/rigs/${id}/agents`), {
      method: 'POST',
      headers: headers(),
    ...
    const heartbeatRes = await SELF.fetch(
      api(`/api/towns/${tid}/rigs/${id}/agents/${agentId}/heartbeat`),
      {
        method: 'POST',
        headers: headers(),
    ...
    const getRes = await SELF.fetch(api(`/api/towns/${tid}/rigs/${id}/agents/${agentId}`), {
      headers: headers(),
    });
