...
import {
  ...
  BITBUCKET_MAX_REQUEST_TIMEOUT_MS,
  fetchBitbucket,
  hasNonVisibleAscii,
  readBoundedBitbucketBody,
} from './bitbucket-safe-transport.js';

export { BitbucketApiError, type BitbucketApiErrorCode } from './bitbucket-safe-transport.js';
...
async function fetchRepositoryPage(
  ...
  const { response, signal } = await fetchBitbucket(endpoint, {
    accessToken,
    resourcePath: new URL(endpoint).pathname,
  ...
  const body = await readBoundedBitbucketBody(response, signal);
  let payload: unknown;
  try {
    payload = JSON.parse(new TextDecoder('utf-8', { fatal: true, ignoreBOM: false }).decode(body));
  } catch {
    throw new BitbucketApiError('invalid_response');
  }
