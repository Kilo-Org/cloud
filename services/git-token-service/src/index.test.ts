...
describe('GitTokenRPCEntrypoint GitHub session capability RPCs', () => {
  ...
  it('redeems a user-pinned capability for api.github.com to preserve broad gh compatibility', async () => {
    const service = createService();
    const issued = await service.issueGitHubSessionCapability({
      githubRepo: 'acme/repo',
      userId: 'user_1',
...
describe('GitTokenRPCEntrypoint GitLab session capability RPCs', () => {
  ...
  it.each([
    [
      'GET',
      'https://gitlab.example.com/acme/platform/widgets.git/info/refs?service=git-upload-pack',
      {
  ...
  it.each([
    ...
    ['GET', 'https://gitlab.com/api/v4/projects?membership=true', 'Bearer'],
    ['POST', 'https://gitlab.com/api/graphql', 'Bearer'],
