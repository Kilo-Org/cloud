...
describe('authMiddleware', () => {
  ...
  it.each([
    ['another batch', NOTIFICATIONS_AUDIENCE],
  ...
  it.each([null, '', ['kilo-chat', 'kilo-chat'], ['kilo-chat', 1], []])(
  ...
  it.each([
    ...
    const { response } = await request(await signToken({ audience: KILO_CHAT_AUDIENCE, pepper }));
  ...
  it.each([
    ['a blocked account', () => (userRow.blockedReason = 'manual block'), 'production'],
    ['a missing env claim', () => undefined, undefined],
    ['a mismatched environment', () => undefined, 'development'],
  ])('returns 401 for %s', async (_name, arrange, tokenEnv) => {
    arrange();
    const { response } = await request(
      await signToken({ audience: KILO_CHAT_AUDIENCE, env: tokenEnv })
    );
    expect(response.status).toBe(401);
  });
  ...
  it('maps dependency failures to 401', async () => {
    dbState.fails = true;
    const { response } = await request(await signToken({ audience: KILO_CHAT_AUDIENCE }));
    expect(response.status).toBe(401);
  });
});
