...
/**
 * The one upstream path `call`, `call_protected` and `submit_otp` share:
 * forward the call to apps/web over its public tRPC transport, chosen by the
 * catalog row's `kind`, with the grant credentials (never a caller-supplied
 * header), and serialize the result under the byte cap.
 *
 ...
 * user's approval; every rejection decidable locally (unknown path,
 * schema-invalid input) is thrown before this runs, so this helper never
 * re-decides access.
 */
