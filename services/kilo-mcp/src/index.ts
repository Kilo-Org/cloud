...
async function runTool(
  ...
  if (name === 'search') {
    ...
    if (results.length === 0) {
      ...
      // is told the endpoints exist — a non-admin gets no admin/debug trace. The
      // echoed query is bounded before serialization — never passed to the byte
      // cap, which would cut the structured payload into invalid JSON.
