...
export interface OAuthStoreApi {
  registerClient(input: NewOAuthClient): Promise<void>;
  ...
  /** Atomic rotation: revoke the old token (if still valid) and insert the new one. */
  rotateRefreshToken(oldId: string, input: NewRefreshToken, nowIso: string): Promise<boolean>;
  /**
...
export class KiloMcpOAuthStore extends DurableObject<Env> implements OAuthStoreApi {
  ...
  async registerClient(input: NewOAuthClient): Promise<void> {
    this.db
      .insert(oauthClients)
      .values({
  ...
  async purgeExpired(nowIso: string): Promise<number> {
    ...
    const expiredJtis = this.db
      ...
      .all().length;
    return expiredCodes + expiredRefreshTokens + expiredJtis;
  }
