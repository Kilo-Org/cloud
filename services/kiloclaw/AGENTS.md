...
- **Next.js owns the Postgres registry; the Worker writes only narrow operational metadata.** Next.js writes registry data (instance creation, ownership, config backup, lifecycle decisions). The Worker reads broadly via Hyperdrive (pepper validation, DO restore) and writes Postgres in two specific carve-outs: (1) destroy finalization (`markDestroyedInPostgresHelper` — closes out a row when the DO has confirmed Fly resources are gone), and (2) denormalized operational metadata that needs to be SQL-queryable for admin tooling (currently `tracked_image_tag`, written by the alarm reconciler). Authoritative state for these columns still lives in the DO; the Postgres copy is a denormalized read cache. New Worker writes beyond these carve-outs require explicit justification — prefer a Next.js tRPC procedure unless the data fundamentally lives in the DO and is being denormalized for query-shape reasons.
