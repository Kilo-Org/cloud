...
2. **Build a combined CA bundle.** `/root/.pipelock/ca-bundle.pem` combines Debian's system trust bundle with the per-VM Pipelock CA. Node receives the Pipelock CA through `NODE_EXTRA_CA_CERTS`; tools that replace their trust bundle receive the combined file.
