...
const pluginBase = createChannelPluginBase({
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      '- `send`: pass `message` plus `target`; `conversationId` and `groupId` are accepted compatibility aliases. To send an attachment from generated bytes, pass base64 `buffer`, `filename`, and `contentType`.',
      '- `upload-file`: preferred for file attachments; pass `target`/`conversationId`, plus `buffer` or `media`/`path`/`filePath`, `filename`, `contentType`, and optional `message` caption.',
