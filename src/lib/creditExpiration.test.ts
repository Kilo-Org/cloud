...
import {
  ...
  retroactivelyExpireCreditsForUser,
...
describe('retroactivelyExpireCreditsForUser', () => {
  const insertMigratedTestUser = async (
    userData: Partial<typeof kilocode_users.$inferInsert> = {}
  ) => {
    const user = defineTestUser(userData);
  ...
  it('returns null when no free transactions exist', async () => {
    const user = await insertMigratedTestUser({
      microdollars_used: 0,
      total_microdollars_acquired: 1_000_000,
    });

    // Only paid transactions — nothing to expire
    await db.insert(creditTransactionsTable).values({
      kilo_user_id: user.id,
    ...
    const result = await retroactivelyExpireCreditsForUser(user, new Date('2025-01-15'));

    expect(result).toBeNull();
  });

  it('expires unused credits and returns correct balance change', async () => {
    const user = await insertMigratedTestUser({
      microdollars_used: 0,
    ...
    expect(result).not.toBeNull();
    expect(result!.balanceBefore).toBe(5_000_000);
    ...
    const expirationTxns = await db
      ...
      .where(eq(creditTransactionsTable.credit_category, 'credits_expired'));
    expect(expirationTxns).toHaveLength(1);
    expect(expirationTxns[0].amount_microdollars).toBe(-5_000_000);
  });

  it('preserves used portion of credits', async () => {
    const user = await insertMigratedTestUser({
    ...
    expect(result).not.toBeNull();
    // Balance before: 5M - 3M = 2M
    expect(result!.balanceBefore).toBe(2_000_000);
    // After: used 3M of 5M, so 2M expired, balance = 0
    expect(result!.balanceAfter).toBe(0);
  });

  it('does not touch paid credits', async () => {
    const user = await insertMigratedTestUser({
      microdollars_used: 0,
    ...
    expect(result).not.toBeNull();
    expect(result!.balanceBefore).toBe(8_000_000);
    // Only free (5M) expired; paid (3M) untouched
    expect(result!.balanceAfter).toBe(3_000_000);
  });

  it('skips transactions that already have an expiry_date', async () => {
    const user = await insertMigratedTestUser({
      microdollars_used: 0,
      total_microdollars_acquired: 5_000_000,
      next_credit_expiration_at: '2025-06-01T00:00:00Z',
    });

    // Transaction already has an expiry date — should NOT be picked up
    await db.insert(creditTransactionsTable).values({
      kilo_user_id: user.id,
      amount_microdollars: 5_000_000,
      is_free: true,
      credit_category: 'promo_a',
      description: 'Already expiring',
  ...
  it('expires free credits with null credit_category', async () => {
    const user = await insertMigratedTestUser({
      microdollars_used: 0,
      total_microdollars_acquired: 5_000_000,
    });
    ...
    await db.insert(creditTransactionsTable).values({
      ...
      description: 'Legacy credits without category',
      original_baseline_microdollars_used: 0,
    });
  ...
  it('expires all free credits with different baselines from realistic grant timeline', async () => {
    // Simulate: user gets credits at different points, uses some, then gets blocked
    const user = await insertMigratedTestUser({
    ...
    await db.insert(creditTransactionsTable).values([
      ...
      },
      {
        ...
        original_baseline_microdollars_used: 1_000_000,
    ...
    expect(result).not.toBeNull();
    // Balance before: 10M - 2M = 8M
    expect(result!.balanceBefore).toBe(8_000_000);
    // All free credits should be fully expired (unused portion = total - used = 10M - 2M = 8M)
    expect(result!.balanceAfter).toBe(0);
  });
  ...
  it('is idempotent — second call returns null', async () => {
    ...
    const now = new Date('2025-01-15');
    await retroactivelyExpireCreditsForUser(user, now);

    // Re-fetch user for second call
    const updatedUser = await db.query.kilocode_users.findFirst({
      where: eq(kilocode_users.id, user.id),
    });

    // Second call: the transaction now has an expiry_date, so no candidates
    const result2 = await retroactivelyExpireCreditsForUser(updatedUser!, now);
    expect(result2).toBeNull();

    // Still only one expiration transaction
    const expirationTxns = await db
      .select()
      .from(creditTransactionsTable)
      .where(eq(creditTransactionsTable.credit_category, 'credits_expired'));
