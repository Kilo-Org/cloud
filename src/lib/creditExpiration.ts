...
/**
 * Retroactively expire all unprocessed free credit transactions for a user.
 * Sets expiry_date to `now` on qualifying transactions, then runs
 * processLocalExpirations to settle the balance.
 *
 * Targets all free, positive-amount, non-expiring, non-organization
 * transactions — including those with a NULL credit_category.
 *
 * Returns the user's balance (total_microdollars_acquired - microdollars_used)
 * before and after, or null if no matching transactions were found.
 */
export async function retroactivelyExpireCreditsForUser(
  user: UserForLocalExpiration,
  now: Date
): Promise<{ balanceBefore: number; balanceAfter: number } | null> {
  const expiredCredits = alias(creditTransactionsTable, 'expired_credits');

  // Find all unprocessed, non-expiring, free credit transactions
  const candidates = await db
    ...
    .leftJoin(
      expiredCredits,
      and(
        eq(expiredCredits.kilo_user_id, user.id),
        inArray(expiredCredits.credit_category, [
          'credits_expired',
          'orb_credit_expired',
          'orb_credit_voided',
    ...
    .where(
      and(
        eq(creditTransactionsTable.kilo_user_id, user.id),
  ...
  // Stamp expiry_date and expiration_baseline on all matching transactions,
  // copying from original_baseline where expiration_baseline is not yet set.
  await db
    .update(creditTransactionsTable)
    .set({
      expiry_date: nowIso,
      expiration_baseline_microdollars_used: sql`COALESCE(${creditTransactionsTable.expiration_baseline_microdollars_used}, ${creditTransactionsTable.original_baseline_microdollars_used}, 0)`,
    })
    .where(inArray(creditTransactionsTable.id, candidateIds));

  // Update next_credit_expiration_at so processLocalExpirations will fire
  await db
    .update(kilocode_users)
    .set({
      next_credit_expiration_at: sql`COALESCE(LEAST(${kilocode_users.next_credit_expiration_at}, ${nowIso}), ${nowIso})`,
    })
    .where(eq(kilocode_users.id, user.id));

  // Re-fetch user to get updated fields for processLocalExpirations
  const freshUser = await db.query.kilocode_users.findFirst({
    where: eq(kilocode_users.id, user.id),
    columns: {
      id: true,
      microdollars_used: true,
      next_credit_expiration_at: true,
      updated_at: true,
  ...
  const balanceBefore = freshUser.total_microdollars_acquired - freshUser.microdollars_used;

  const expirationResult = await processLocalExpirations(freshUser, now);

  const balanceAfter = expirationResult
    ? expirationResult.total_microdollars_acquired - freshUser.microdollars_used
    : balanceBefore;
