...
function getRandomNumberLessThan100(userId: string | undefined) {
  return userId
    ? crypto.createHash('sha256').update(userId).digest().readUInt32BE(0) % 100
...
export async function shouldRouteToVercel(
  ...
  return getRandomNumberLessThan100(userId) < VERCEL_ROUTING_PERCENTAGE;
}
